...
public class MemoryBasedKeyStore implements FloodControlKeyStore {
	...
	/**
	 * Note: Package level access here is intentional. (The absence of public, private, or protected.) Callers are directed to use the FloodControlKeyStoreFactory to create key stores.
	 * 
	 * @param props
	 *            Properties required to initialize the keystore.
	 ...
	 */
	static MemoryBasedKeyStore getInstance(Properties props) throws FloodControlInitException {
		if (obj == null) {
			obj = new MemoryBasedKeyStore(props);
		}
		return obj;
	}
	...
	/**
	 * Private constructor for the MemoryBasedKeyStore, enforces use of the Singleton pattern.
	 * 
	 * @param props
	 *            Properties required to initialize the keystore.
	 * @throws FloodControlInitException
	 *             for any mechanical fault that occurs during initialization.
	 */
	private MemoryBasedKeyStore(Properties props)
			throws FloodControlInitException {
		...
		if (maxConcurrentKeys < MINIMUM_HASHTABLE_SIZE) {
			throw new FloodControlInitException(
					"FloodControl.MaxConcurrentKeys property value below minimum of " + MINIMUM_HASHTABLE_SIZE + ".");
		}
		...
		try {
			...
			throw new FloodControlInitException(
					"Unable to recreate flood control keyStore." + kse
							.toString());
		}
	...
	/**
	 * Creates (or resets) the memory keystore. In this case, the keystore is a Hashtable.
	 * 
	...
	private synchronized void initialize() throws KeyStoreException {
		Double scalingFactor = new Double(
				maxConcurrentKeys * DATA_STRUCTURE_SCALING_FACTOR);
	...
	/**
	 * @see pvt.disney.dti.gateway.util.flood.FloodControlKeyStore#addKeyOccurrence(Object key)
	 */
	...
	/**
	 * @see pvt.disney.dti.gateway.util.flood.FloodControlKeyStore#addBlockingKeyOccurrence(Object key)
	 */
	...
	/**
	 * This method is meant to be called internally by another method which has already established a lock.
	 * 
	 * @param key
	...
	private void addKeyOccurrence(Object key, boolean isBlocked) throws KeyStoreException {
		...
		if (isBlocked) aKeyRecord.setBlockedKey(isBlocked);
	...
	/**
	 * @see pvt.disney.dti.gateway.util.flood.FloodControlKeyStore#retrieveKeyOccurrences(Object key)
	 */
	public Date[] retrieveKeyOccurrences(Object key) throws KeyStoreException {
		...
		if (dateList.length == 0) return new Date[0];

		return dateList;
	}

	/**
	 * @see pvt.disney.dti.gateway.util.flood.FloodControlKeyStore#storeNewKey(Object key)
	 */
	public synchronized void storeNewKey(Object key) throws KeyStoreException {
		...
		try {
			...
			// MemoryBasedKeyStore
			// upon a later evaluation to determine if the blocking threshold has been
			// passed.
			if (anOldKeyRecord == null) return;
			else {
	...
	/**
	 * @see pvt.disney.dti.gateway.util.flood.FloodControlKeyStore#clearOccurrencesOlderThan(Date expireTime, Object key)
	 */
	public int clearOccurrencesOlderThan(Date expireTime, Object key) throws KeyStoreException {
		...
		if (aKeyRecord == null) return 0;
		else numberCleared = aKeyRecord.getNumberOfOccurrences();
		...
		try {
			numberCleared = aKeyRecord.clearOccurrencesOlderThan(expireTime,
					key);
		}
	...
	/**
	 * @see pvt.disney.dti.gateway.util.flood.FloodControlKeyStore#refreshKeyStore(Date occurrenceExpireTime)
	 */
	public int refreshKeyStore(Date occurrenceExpireTime) throws KeyStoreException {

		int howManyCleared = 0;

		if (setRefreshingKeyStore() == false) return 0;
		...
		try {
			...
			for (int i = 0; i < keyArray.length; i++) {
				howManyCleared = clearOccurrencesOlderThan(
						occurrenceExpireTime, keyArray[i]);
			}
		}
		finally {
	...
	/**
	 * @see pvt.disney.dti.gateway.util.flood.FloodControlKeyStore#countKeyOccurrences(Object key)
	 */
	...
	/**
	 * @see pvt.disney.dti.gateway.util.flood.FloodControlKeyStore#isBlockedKey(Object key)
	 */
	public boolean isBlockedKey(Object key) throws KeyStoreException {
		KeyRecord aKeyRecord = keyStore.get(key);

		if (aKeyRecord == null) return false;
		else return aKeyRecord.isBlockedKey();
	}

	/**
	 * @see pvt.disney.dti.gateway.util.flood.FloodControlKeyStore#removeBlockedKeysOlderThan(Date blockExpireTime)
	 */
		...
		try {
			...
			for (int i = 0; i < keyArray.length; i++) {
				...
				synchronized (this) {
					...
					// If this record has just been removed by another thread, then exit.
					if (aKeyRecord == null) continue;

					// if this record is not in a blocked state, then move to the next
					// record.
					if (!aKeyRecord.isBlockedKey()) {
						continue;
					}
	...
	/**
	 * Synchronized method to ensure a lock is released when refreshing the keystore.
	 */
	...
	/**
	 * Synchronized method to ensure atomic, non-parallel key store refreshes, ensuring only one thread gets the lock.
	 * 
	 * @return
	 */
	private synchronized boolean setRefreshingKeyStore() {
		if (refreshingKeyStore == true) return false;
		else {
	...
	/**
	 * Synchronized method to ensure a lock is released when clearing blocked keys.
	 * 
	 */
	...
	/**
	 * Synchronized method to ensure atomic, non-parallel clearing of blocked keys, ensuring only one thread gets the lock.
	 */
	private synchronized boolean setClearingBlocks() {
		if (clearingBlocks == true) return false;
		else {
			clearingBlocks = true;
			return true;
		}
	...
	/**
	 * Exception used only within the confines of the MemoryBasedKeyStore to show that key is not empty of all occurrences.
	 * 
	 * @author lewit019
	 * 
	 */
	...
	/**
	 * A class that encapsulates a key record within the confines of the MemoryBasedKeyStore. This class is internal to ensure adequate encapsulation and that visibility of the innerworkings of the implementation remain opaque.
	 * 
	 * @author lewit019
	 */
	private class KeyRecord {
		...
		/**
		 * Constructor that creates each key record with with one occurrence record already pre-populated.
		 */
		public KeyRecord() {
			Date rightNow = new Date();
		...
		/**
		 * Sets the key to blocked if the parameter is set to true. If it is set to true, it records the date/time it was blocked so other processes can obtain that information.
		 * 
		 * @param isBlocked
		...
		/**
		 * Clears occurrences on a key record older than the expire time specified.
		 * 
		 * @param expireTime
		 ...
		 */
		public synchronized int clearOccurrencesOlderThan(Date expireTime,
				Object key) throws EmptyKeyException {

			boolean stop = false;
			int numberCleared = 0;
			int index = 0;

			// If any occurrence time is older (less) than the expireTime, age it off.
			while (stop == false) {

				if (occurrences.size() == 0) break;

				if (occurrences.get(index).getTime() < expireTime.getTime()) {
					occurrences.remove(index);
					numberCleared++;
					if (occurrences.size() == 0) stop = true;
				}
				else {
					index++;
					if (index == occurrences.size()) stop = true;
				}
