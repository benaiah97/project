...
public class DLRQueryEligibilityProductsRules implements TransformConstants {
   ...
   /**
    * Transforms a Query Eligible Product ticket response string from the DLR
    * provider and updates the DTITransactionTO object with the response
    * information.
    * 
    * @param dtiTxn
   ...
   static DTITransactionTO transformResponse(DTITransactionTO dtiTxn, String xmlResponse) throws DTIException {
      ...
      // If the provider had an error, map it and generate the response. Copy
      // the ticket identity and default the TktStatus Voidable to No
      if (statusCode != 0) {
      ...
      // GuestProductTO
      GuestProductTO guestProduct = new GuestProductTO();

      // Putting up the original response in GuestProductTO
      guestProduct.setGwDataRespTO(gwDataRespTO);

      // PLU's obtained from GWDataresponse , throw error if no PLU found.
      if ((gwDataRespTO.getPlu() == null)) {

         logger.sendEvent("Provider returned no PLU on a successful query ticket. ", EventType.EXCEPTION, THISINSTANCE);
         throw new DTIException(DLRQueryEligibilityProductsRules.class, DTIErrorCode.TP_INTERFACE_FAILURE,
                  "Provider returned no PLU on a successful query ticket. ");
      } else {
         ...
         if (guestDbProduct == null) {
            logger.sendEvent("No product found in DB for given PLU." + gwDataRespTO.getPlu(), EventType.DEBUG,
                     THISINSTANCE);
         ...
         ArrayList<UpgradePLU> upGradedPLuList = guestProduct.getGwDataRespTO().getUpgradePLUList();
         ...
         if ((upGradedPLuList == null) || (upGradedPLuList.size() == 0)) {
            ...
            for (/* each */UpgradePLU upgradePLU : /* in */upGradedPLuList) {
               if (upgradePLU.getPaymentPlans() != null) {

                  // Pay Plan Eligibility Status will be YES if payment Plan is
                  // found in any of Upgraded PLU's List
                  dtiTktTO.setPayPlanEligibilityStatus(PayPlanEligibilityStatusType.YES);
                  continue;
               }
            }
            ...
            if (globalUpgradeProduct.getProductListCount() == 0) {
               ...
               // Mapping the globalGuestProduct and globalUpgradeProduct to
               // response.
               setQueryEligibleResponseCommand(guestProduct, dtiTktTO, globalUpgradeProduct.getProductList());
            }
         }
      }
      return setQueryEligibleToDTITransaction(dtiTktTO, dtiRespTO, dtiTxn);
    ...
    */
   private static DTITransactionTO setQueryEligibleToDTITransaction(TicketTO dtiTktTO, DTIResponseTO dtiRespTO,
   ...
   static DTITransactionTO transformError(DTITransactionTO dtiTxn, DTIResponseTO dtiRespTO, String statusCode,
      ...
      // Copy over key ticket values for error response to match existing
      // format.
   ...
   private static void setQueryEligibleResponseCommand(GuestProductTO guestProductTO, TicketTO dtiTktTO,
      ...
      if (dbProductTO != null) {
         ...
         // Check if the dbProductTO is populated from DB or from Galaxy
         if (guestProductTO.isSourcedFromDB()) {

            // Prod Code if PLU is found in DTI DB
            dtiTktTO.setProdCode(dbProductTO.getPdtCode());
         } else {

            // DLRPLU, if PLU is not found in DTI, pulling PLU information from
            // Galaxy response.
            dtiTktTO.setDlrPLU(dbProductTO.getDlrPLU());
         }
      ...
      if ((upgradedProductTOList != null) && (upgradedProductTOList.size() > 0)) {
         for (/* each */DBProductTO productTO : /* in */upgradedProductTOList) {
            ...
            logger.sendEvent("Setting all the products/eligible products to DTI Ticket Transaction Object  ",
   ...
   /**
    * Gets the PLU details from galaxy. This will use to retain PLU details from
    * Galaxy response in case if no matching value for PLU is not found in DTI.
    *
    * @param guestProductTO
    *           the guest product TO
    * @param dtiTktTO
    *           the dti tkt TO
    * @return the PLU details from galaxy
    */
   private static DBProductTO getPLUDetailsFromGalaxy(GuestProductTO guestProductTO, TicketTO dtiTktTO) {
      GWDataRequestRespTO gwDataRespTO = guestProductTO.getGwDataRespTO();

      // Comparing the value of itemKind in galaxy response
      if (gwDataRespTO.getItemKind().compareTo(ItemKind.REGULAR_TICKET) != 0) {

         logger.sendEvent("If the itemKind is not 1 mark the ticket as INELIGIBLE.", EventType.DEBUG,
                  dtiTktTO.toString());
         dtiTktTO.setUpgradeEligibilityStatus(UpgradeEligibilityStatusType.INELIGIBLE);
      }

      // SourcedFromDB, determine if the value is populated from Galaxy or DTI.
      guestProductTO.setSourcedFromDB(false);

      // Store PLU details from Galaxy response in dbProductTO,
      DBProductTO dbProductTO = new DBProductTO();

      // Map PLU to new tag DLRPLU
      String dlrPLU = gwDataRespTO.getPlu();

      if (dlrPLU != null) {
         dbProductTO.setDlrPLU(dlrPLU);
      }

      // ProdGuestType
      dbProductTO.setGuestType(GuestType.ANY);

      // SRP Price
      dbProductTO.setStandardRetailPrice(gwDataRespTO.getPrice());

      // SRP Tax
      dbProductTO.setStandardRetailTax(gwDataRespTO.getTax());

      // TktValidity ValidStart and ValidEnd
      if ((gwDataRespTO.getDateSold() != null) || (gwDataRespTO.getTicketDate() != null)
               || (gwDataRespTO.getStartDateTime() != null)) {
         dbProductTO.setStartSaleDate(gwDataRespTO.getDateSold());
      }

      if ((gwDataRespTO.getExpirationDate() != null) || (gwDataRespTO.getEndDateTime() != null)) {
         dbProductTO.setEndValidDate(gwDataRespTO.getExpirationDate());
      }

      // MappedProviderTktName
      dbProductTO.setMappedProviderTktName(dlrPLU);
