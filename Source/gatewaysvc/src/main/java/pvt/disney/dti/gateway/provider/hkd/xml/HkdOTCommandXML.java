...
import pvt.disney.dti.gateway.provider.hkd.data.common.HkdOTCreditCardTO.CreditCardEntryType;
 ...
 * @since 2.16.3
 */
public class HkdOTCommandXML implements TransformConstants {
   ...
   */
  public static String getXML(HkdOTCommandTO otCommandTO) throws DTIException {
    ...
    HkdOTCommandTO.OTTransactionType txnType = otCommandTO.getTxnType();

    HkdOTHeaderTO otHeader = otCommandTO.getHeaderTO();
    HkdOTHeaderXML.addHeaderElement(otHeader, commandStanza);
    ...
    case MANAGERESERVATION:
      ...
      HkdOTManageReservationTO otMngResTO = otCommandTO
          .getManageReservationTO();
      HkdOTManageReservationXML.addTxnBodyElement(otMngResTO, commandStanza);
      break;

    default:
      throw new DTIException(HkdOTCommandXML.class,
   ...
   */
  public static HkdOTCommandTO getTO(String xmlResponse) throws DTIException {

    Document document = null;
    HkdOTCommandTO otCmdTO = new HkdOTCommandTO(
        HkdOTCommandTO.OTTransactionType.UNDEFINED);
    HkdOTHeaderTO otHdrTO = null;
    ...
    if (commandStanza.getName().compareTo("Command") != 0) throw new DTIException(
        HkdOTCommandXML.class, DTIErrorCode.TP_INTERFACE_FAILURE,
    ...
    if (headerNode == null) {
      ...
      otHdrTO = HkdOTHeaderXML.getTO(headerNode);
      otCmdTO.setHeaderTO(otHdrTO);

      if (otHdrTO.getRequestSubType().compareTo(OT_MANAGE_RES_TYPE) == 0) {
        otCmdTO.setTxnType(HkdOTCommandTO.OTTransactionType.MANAGERESERVATION);
      }
      else {
      ...
      if (manageResNode != null) {
        HkdOTManageReservationTO otMngRes = HkdOTManageReservationXML
            .getTO(manageResNode);
      ...
      if (!bodyElementFound) throw new DTIException(HkdOTCommandXML.class,
          DTIErrorCode.TP_INTERFACE_FAILURE,
  ...
  static void addPaymentInfoStanza(Element txnStanza,
      ArrayList<HkdOTPaymentTO> payList) {

    if ((payList == null) || (payList.size() == 0)) return;

    for /* each */(HkdOTPaymentTO aPaymentTO : /* in */payList) {
   ...
   */
  static void addCreditCardStanza(Element payStanza, HkdOTCreditCardTO ccardTO) {

    Element ccardStanza = payStanza.addElement("CreditCard");

    Element ccInfoStanza = ccardStanza.addElement("CardInfo");

    if (ccardTO.getCcManualOrSwiped() == HkdOTCreditCardTO.CreditCardEntryType.SWIPED) {
   ...
   */
  static void addVoucherStanza(Element payStanza, HkdOTVoucherTO voucherTO) {
  ...
  static void addInstallmentStanza(Element payStanza,
    ...
    payorStanza.addElement("ClientUniqueId").addText(
        Integer.toString(HkdOTInstallmentTO.getClientuniqueid()));
    Element demoDataStanza = payorStanza.addElement("DemographicData");
    ArrayList<HkdOTFieldTO> oTFieldList = installTO.getDemographicData();

    HkdOTCommonXML.addOTDemoAsFieldType(demoDataStanza, oTFieldList);
  ...
  static void addInTransactionAttributeStanzas(Element tktStanza,
      ArrayList<HkdOTInTransactionAttributeTO> inTxnAttrList) {

    if (inTxnAttrList.size() > 0) {

      for /* each */(HkdOTInTransactionAttributeTO otAttr : /* in */inTxnAttrList) {
   ...
   */
  static HkdOTErrorTO setOTErrorTO(Node errorNode) throws DTIException {

    HkdOTErrorTO errTO = new HkdOTErrorTO();
   ...
   */
  static HkdOTTicketTO setOTTicketTO(Node ticketNode) throws DTIException {

    HkdOTTicketTO otTicket = new HkdOTTicketTO();
   ...
   */
  static void setOTTicketInfoValidity(Node aNode, HkdOTTicketInfoTO otTktTO) throws DTIException {
   ...
   */
  static HkdOTTransactionDSSNTO setOTTransactionDSSNTO(Node dssnNode) throws DTIException {

    HkdOTTransactionDSSNTO otTranDssn = new HkdOTTransactionDSSNTO();
    ...
    try {
      ...
      throw new DTIException(HkdOTManageReservationXML.class,
          DTIErrorCode.TP_INTERFACE_FAILURE,
   ...
   */
  static void setOTPaymentTOList(ArrayList<HkdOTPaymentTO> otPayList,
    ...
    for /* each */(Node aNode : /* in */payList) {
      ...
      HkdOTPaymentTO otPayTO = new HkdOTPaymentTO();
   ...
   */
  private static HkdOTCreditCardTO setOTCreditCardTO(Node ccNode) throws DTIException {
   ...
   */
  private static HkdOTVoucherTO setOTVoucherTO(Node voucherNode) throws DTIException {

    HkdOTVoucherTO otVoucher = new HkdOTVoucherTO();
   ...
   *             for any missing mandatory fields or other parsing errors.
   */
  private static HkdOTInstallmentTO setOTInstallmentTO(Node instNode) throws DTIException {
   ...
   */
  public static HkdOTErrorTO getErrorTO(Node bodyNode) throws DTIException {

    // Error
    Node errorNode = bodyNode.selectSingleNode("Error");
    if (errorNode == null) throw new DTIException(HkdOTCommandXML.class,
        DTIErrorCode.TP_INTERFACE_FAILURE,
        "Ticket provider returned XML without a Error element.");

    // ItemStatus replacement for generic errors, if required.
    HkdOTErrorTO otErrorTO = HkdOTCommandXML.setOTErrorTO(errorNode);
    if ((otErrorTO.getErrorCode().intValue() == HkdOTCommandXML.GENERIC_OPER_PARTIALLY_SUCCESSFUL) || (otErrorTO
        .getErrorCode().intValue() == HkdOTCommandXML.GENERIC_OPER_FAILED)) {
