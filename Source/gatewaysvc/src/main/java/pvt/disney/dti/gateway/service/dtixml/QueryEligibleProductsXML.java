...
public class QueryEligibleProductsXML {
	...
	public static QueryEligibleProductsRequestTO getTO(
		...
		QueryEligibleProductsRequest.Ticket aTicket = queryReq.getTicket();
		...
		queryEligTo.setTicketTO(aTicketTO);
		// Sale Type
		queryEligTo.setSaleType(aTicket.getSaleType());
		return queryEligTo;
	...
	public static QueryEligibleProductsResponse getJaxb(
		...
		TicketTO aTicketTO = qryResRespTO.getTicketTO();
		...
		if (typeList.size() > 0) {
			...
			/*
			 * // Mag Code? if (typeList.contains(TicketTO.TicketIdType.MAG_ID))
			 * { String tktMagTO = aTicketTO.getMagTrack1(); qName = new
			 * QName("Mag"); JAXBElement<String> tktMag = new JAXBElement(qName,
			...
			/*
			 * // Bar Code? if
			 * (typeList.contains(TicketTO.TicketIdType.BARCODE_ID)) { String
			 * tktBarTO = aTicketTO.getBarCode(); qName = new QName("Barcode");
		...
		// EligibleProducts
		if (aTicketTO.getProdCode() != null && aTicketTO.getProdPrice() != null) {
