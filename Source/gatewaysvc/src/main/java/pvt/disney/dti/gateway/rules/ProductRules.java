...
/**
 * The class ProductRules is responsible for implementing the various product
 * rules such as availability, correct shells, etc.
 * 
 * @author lewit019
 * @since 2.16.3
 */
public class ProductRules {
   ...
   */
  public static void validateProductsAreSellable(ArrayList<TicketTO> tktListTO, ArrayList<DBProductTO> dbProdList)
      throws DTIException {

    validateProductsAreSellable(tktListTO, dbProdList, ValueConstants.TYPE_CODE_SELL);
  ...
  public static void validateProductsAreSellable(ArrayList<TicketTO> tktListTO, ArrayList<DBProductTO> dbProdList,
    ...
    if ((dbProdList == null) || (dbProdList.size() == 0) || (typeCode == null)) {
    ...
    if (productCodeSet.size() != dbProdList.size()) {
      throw new DTIException(ProductRules.class, DTIErrorCode.INVALID_PRODUCT_CODE,
    ...
    for /* each */(DBProductTO aProduct : /* in */dbProdList) {
      ...
      if (!aProduct.isActive()) {
        throw new DTIException(BusinessRules.class, DTIErrorCode.PRODUCT_UNAVAILABLE, "Product "
            + aProduct.getPdtCode() + " is not active in the database.");
      }

      if (aProduct.isSoldOut()) {
        throw new DTIException(ProductRules.class, DTIErrorCode.PRODUCT_SOLD_OUT, "Product " + aProduct.getPdtCode()
            + " is sold out in the database.");
      }
      ...
      if (!DateTimeRules.isNowWithinDate(startSaleDate, endSaleDate)) {
        throw new DTIException(ProductRules.class, DTIErrorCode.PRODUCT_EXPIRED, "Product " + aProduct.getPdtCode()
            + " can't be sold because of start and/or end sale dates.");
      }
   ...
   * @since 2.12
   */
  public static void validateOneShipProduct(ArrayList<TicketTO> tktListTO, ArrayList<DBProductTO> dbProdList)
      throws DTIException {
    ...
    if (numberOfShippingProducts > 1) {
      throw new DTIException(ProductRules.class, DTIErrorCode.INVALID_PRODUCT_CODE, "Shipping product "
          + shippingProduct + " appears multiple times on this reservation.");
    }
   ...
   *           should any rule fail.
   */
  public static void validateNonZeroProductQuantity(ArrayList<TicketTO> tktListTO) throws DTIException {

    for /* each */(TicketTO aTicketTO : /* in */tktListTO) {

      if ((aTicketTO.getProdQty() == null) || (aTicketTO.getProdQty().longValue() < 1)) {
        throw new DTIException(ProductRules.class, DTIErrorCode.INVALID_PRODUCT_QTY, "Product "
            + aTicketTO.getProdCode() + " can't have a null or < 1 quantity.");
      }
    }
  ...
  /**
   * For each product, determine if it has a valid sales relationship in either
   * the entity X product cross-reference or the entity X product group
   * cross-reference. If one of those products does not have that relationship,
   * then throw an exception.<BR>
   * Enforces the following rules: <BR>
   * 1. Does entity have permission in the database to sell all of the order
   * products?
   * 
   * @param dbProdList
   *          the db prod list
   ...
   *           the DTI exception
   */
  public static void validateEntityCanSellProducts(EntityTO entityTO, ArrayList<DBProductTO> dbProdList,
      ArrayList<BigInteger> allowedPdtIdList) throws DTIException {

    if ((allowedPdtIdList == null) || (allowedPdtIdList.size() == 0)) {
      throw new DTIException(ProductRules.class, DTIErrorCode.PRODUCT_NOT_PERMITTED, "Entity " + entityTO.getTsMac()
          + "/" + entityTO.getTsLocation() + " doesn't have any products or product groups related to it to sell.");
    }

    for /* each */(DBProductTO aDBProduct : /* in */dbProdList) {

      if (!allowedPdtIdList.contains(aDBProduct.getPdtid())) {
        throw new DTIException(ProductRules.class, DTIErrorCode.PRODUCT_NOT_PERMITTED, "Product "
            + aDBProduct.getPdtCode() + " can't be sold because entity " + entityTO.getTsMac() + "/"
            + entityTO.getTsLocation()
            + " doesn't have it as a related product or a product in a related product group.");
  ...
  /**
   * Validates entities can upgrade products 
   * @param entityTO
   * @param dbProdList
   * @param allowedPdtIdList
   * @throws DTIException
   */
  public static void validateEntityCanUpgradeProducts(EntityTO entityTO, ArrayList<DBProductTO> dbProdList,
      ArrayList<BigInteger> allowedPdtIdList) throws DTIException {

    if ((allowedPdtIdList == null) || (allowedPdtIdList.size() == 0)) {
      throw new DTIException(ProductRules.class, DTIErrorCode.PRODUCT_NOT_PERMITTED, "Entity " + entityTO.getTsMac()
          + "/" + entityTO.getTsLocation() + " doesn't have any products or product groups related to it to upgrade.");
    }

    for /* each */(DBProductTO aDBProduct : /* in */dbProdList) {
      if (!allowedPdtIdList.contains(aDBProduct.getPdtid())) {
        throw new DTIException(ProductRules.class, DTIErrorCode.PRODUCT_NOT_PERMITTED, "Product "
            + aDBProduct.getPdtCode() + " can't be upgraded because entity " + entityTO.getTsMac() + "/"
            + entityTO.getTsLocation()
            + " doesn't have it as a related product or a product in a related product group.");
      }
    }
   ...
   *           should any rule fail.
   * 
   * @see ProductRules#validateProductPrice(EntityTO, ArrayList, ArrayList,
   *      String)
   * 
   */
  public static boolean validateProductPrice(EntityTO entityTO, ArrayList<TicketTO> tktListTO,
   ...
   * 3. Is the price mismatch configuration on the product complete in the
   * database?<BR>
   ...
   */
  public static boolean validateProductPrice(EntityTO entityTO, ArrayList<TicketTO> tktListTO,
      ArrayList<DBProductTO> dbProdList, boolean isTaxExempt) throws DTIException {
   ...
   *          true if the seller is tax exempt
   * 
   *          IMPORTANT NOTE: In DTI product administration, there are a couple
   *          of exceptions regarding how product pricing is set up. The
   *          "printed price" used for sales validations sometimes represents
   *          the "retail" value of the sale (i.e. for Disney Stores). Guests
   *          must pay upgrades against the Unit/Net price or the Standard
   *          Retail Price, only. Actual retail prices are irrelevant in the
   *          upgrade process. (JTL per Craig Stuart)
   * 
   * @throws DTIException
   *           should any rule fail.
   */
  public static void validateUpgradeProductPrice(EntityTO entityTO, ArrayList<TicketTO> tktListTO,
      ArrayList<DBProductTO> dbFromProdList) throws DTIException {
    ...
    for /* each */(TicketTO aTicketTO : /* in */tktListTO) {
      ...
      if (dbFromUnitPrice == null) {
        throw new DTIException(ProductRules.class, DTIErrorCode.DTI_CONFIG_ERROR, "Product code "
            + dbFromProduct.getPdtCode()
            + " has no Unit Price set in the database.  Please refer to Ticket Administration.");
      }

      if (dbFromSRPrice == null) {
        throw new DTIException(ProductRules.class, DTIErrorCode.DTI_CONFIG_ERROR, "Product code "
            + dbFromProduct.getPdtCode()
            + " has no Standard Retail Price set in the database.  Please refer to Ticket Administration.");
      }
      ...
      if ((ticketFromPdtPrice.compareTo(dbFromUnitPrice) != 0) && (ticketFromPdtPrice.compareTo(dbFromSRPrice) != 0)) {
        ...
        throw new DTIException(ProductRules.class, DTIErrorCode.INVALID_PRICE, "Entity " + entityTO.getTsMac() + "/"
            ...
            + dbFromUnitPrice.toString() + " (NET) and " + dbFromSRPrice.toString() + " (Standard Retail Price).");
      }
  ...
  /**
   * Validates if a price falls correctly within the price mismatch range. Note:
   * Mismatching price ranges are inclusive.
   * 
   * @param dbProduct
   *          the db product
   * @param aTicketTO
   ...
   *           should any rule fail.
   */
  private static boolean validatePriceMismatch(TicketTO aTicketTO, DBProductTO dbProduct, BigDecimal validPrice,
    ...
    DBProductTO.MismatchToleranceType mismatchType = dbProduct.getMisMatchTolType();
    ...
    if (ticketPrice.compareTo(highestPriceAllowed) > 0) {
      ...
      if (isTaxExempt) {
        throw new DTIException(ProductRules.class, DTIErrorCode.PRICE_MISMATCH_ERROR_HI, "Product "
            + dbProduct.getPdtCode() + " can't be sold for more than (tax exempt adjusted) "
            + highestPriceAllowed.toString() + " (sale at " + aTicketTO.getProdPrice().toString() + " was attempted.)");
      } else {
        throw new DTIException(ProductRules.class, DTIErrorCode.PRICE_MISMATCH_ERROR_HI, "Product "
            + dbProduct.getPdtCode() + " can't be sold for more than " + highestPriceAllowed.toString() + " (sale at "
            + aTicketTO.getProdPrice().toString() + " was attempted.)");
      }
    }
    ...
    if (ticketPrice.compareTo(lowestPriceAllowed) < 0) {
      ...
      if (isTaxExempt) {
        throw new DTIException(ProductRules.class, DTIErrorCode.PRICE_MISMATCH_ERROR_LO, "Product "
            + dbProduct.getPdtCode() + " can't be sold for less than (tax exempt adjusted) "
            + highestPriceAllowed.toString() + " (sale at " + aTicketTO.getProdPrice().toString() + " was attempted.)");
      } else {
        throw new DTIException(ProductRules.class, DTIErrorCode.PRICE_MISMATCH_ERROR_LO, "Product "
            + dbProduct.getPdtCode() + " can't be sold for less than " + highestPriceAllowed.toString() + " (sale at "
            + aTicketTO.getProdPrice().toString() + " was attempted.)");
      }
    }
   ...
   * Enforces the following rules: <BR>
   * 1. Does the product have an active ticket provider product affiliated with
   * it?
   * 
   * @param dbProdList
   *          the db prod list
   ...
   *           should any rule fail.
   */
  public static void validateMappedProviderTicketTypes(ArrayList<DBProductTO> dbProdList) throws DTIException {

    for /* each */(DBProductTO aDBProduct : /* in */dbProdList) {

      if (aDBProduct.getMappedProviderTktName() == null) {
        throw new DTIException(ProductRules.class, DTIErrorCode.MISSING_TICKET_TYPE, "Product "
            + aDBProduct.getPdtCode() + " is missing its reference to a provider ticket type.");
      }
      if (!aDBProduct.isMappedProviderTktActive()) {
        throw new DTIException(ProductRules.class, DTIErrorCode.INACTIVE_TICKET_TYPE, "Product "
            + aDBProduct.getPdtCode() + " is mapped to inactive ticket type " + aDBProduct.getMappedProviderTktName());
      }
    }
   ...
   * Enforces the following rules: <BR>
   * 1. Does the product require demographics? If so, are they provided in the
   * request? <BR>
   * 2. Do the number of demographics provided equate to the quantity of
   * products? <BR>
   * 3. If a product is NOT supposed to have demographics, it must not have
   * them. <BR>
   ...
   * @since 2.9
   */
  public static void validateProductsHaveDemographics(ArrayList<TicketTO> tktListTO, ArrayList<DBProductTO> dbProdList)
      throws DTIException {
    ...
    for /* each */(TicketTO aTicketTO : /* in */tktListTO) {
      ...
      if (demographicsRequired.booleanValue()) {
        ...
        if ((demoList == null) || (demoList.size() == 0)) {
          throw new DTIException(ProductRules.class, DTIErrorCode.INVALID_TICKET_DEMOGRAPHICS, "Product " + aProdCode
              + " does not have ticket level demographics supplied, and such are required.");
        }
        ...
        if (tktQtyCount != demoCount) {
          throw new DTIException(ProductRules.class, DTIErrorCode.INVALID_TICKET_DEMOGRAPHICS, "Product " + aProdCode
              + " has an invalid number of demographics supplied (" + demoCount + "); does not match prod quantity ("
              + tktQtyCount + ").");
        }
        ...
        if (demoList != null) {
          if (demoList.size() > 0) {
            throw new DTIException(ProductRules.class, DTIErrorCode.INVALID_TICKET_DEMOGRAPHICS, "Product " + aProdCode
                + " does not permit demographics to be supplied, " + "and demographics are present.");
          }
        }
      ...
      for /* each */(DemographicsTO aDemo : /* in */demoList) {
        ...
        if (aDemo.getGenderType() == GenderType.NOTPRESENT) {
          throw new DTIException(ProductRules.class, DTIErrorCode.INVALID_TICKET_DEMOGRAPHICS, "On Product "
              + aProdCode + ", ticket demographics supplied did not meet required "
  ...
  /**
   * Validates if upgrading one entitlement to a different entitlement is valid;
   * Is the to MSRP equal or greater than the from MSRP?
   * 
   * 2.10
   * 
   * @param dbProdList
   ...
   *           should any rule fail.
   */
  public static void validateUpgradePricing(String fromProdCode, BigDecimal fromProdPrice, String toProdCode,
      BigDecimal toProdPrice, BigDecimal upgrdPrice) throws DTIException {

    // RULE: FROM price cannot be greater than TO price. (Corrected by JTL in 2.11)
    if (fromProdPrice.compareTo(toProdPrice) > 0) {
      throw new DTIException(ProductRules.class, DTIErrorCode.INVALID_PRODUCT_UPGRADE_PATH, "From product: "
          + fromProdCode + " at " + fromProdPrice.toString() + " is not allowed to be upgraded to product: "
          + toProdCode + " at " + toProdPrice.toString() + " because the upgrade is to a product of lesser value.");
    }

    // RULE: fromProdPrice + upgrdPrice == prodPrice
    if (toProdPrice.compareTo(fromProdPrice.add(upgrdPrice)) != 0) {
      throw new DTIException(ProductRules.class, DTIErrorCode.INVALID_PRODUCT_UPGRADE_PATH, "From product ("
          + fromProdCode + ") price of " + fromProdPrice.toString() + " plus upgrade amount " + upgrdPrice.toString()
          + " does not equal target product code (" + toProdCode + ") price of " + toProdPrice.toString() + ".");
    }
    ...
    for (/* each */TicketTO aTicketTO : /* in */tktListTO) {
      ...
      if (aTicketTO.getTicketDemoList().size() != 0) {
        ...
        for /* each */(DemographicsTO aDemoTO : /* in */aTicketTO.getTicketDemoList()) {
          ...
          // Validate that if other ticket demographics have been provided,
          // phone has been provided, as well.
  ...
  private static void throwFieldMissExcpt(String park, String field, BigInteger item) throws DTIException {
    ...
    throw new DTIException(ProductRules.class, DTIErrorCode.INVALID_MSG_CONTENT, "For " + park + ", " + field
        + " is a mandatory field for TktDemoData.  TktItem failing was " + item + ".");
  ...
  private static void validateStringDemo(String park, String fieldName, String fieldValue, int minLength,
    ...
    if ((isRequired) && (fieldValue == null)) {
      throw new DTIException(ProductRules.class, DTIErrorCode.INVALID_MSG_CONTENT, "For " + park + ", " + fieldName
          + " is a mandatory field for TktDemoData.  TktItem failing was " + item + ".");
    } else if (!isRequired) {
      return;
    }
    ...
    if ((strLen > maxLength) || (strLen < minLength)) {
      throw new DTIException(ProductRules.class, DTIErrorCode.INVALID_MSG_CONTENT, "For " + park + ", " + fieldName
          + " has invalid length for TktDemoData (expected " + minLength + " to " + maxLength
          + ", but found a length of " + strLen + ").");
    }
