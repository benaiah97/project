...
public class TransidRescodeKey {
	 ...
	 */
	public static final ArrayList<TransidRescode> getTransIdResCode(String transid) throws DTIException {
		return getTransidRescodeFromDB(transid);		
	}
	
	@SuppressWarnings("unchecked")
	public static final ArrayList<TransidRescode> getTransidRescodeFromDB(String transid) throws DTIException  {
		 ArrayList<TransidRescode> result = null; //new ArrayList<TransidRescodeTO>();
			...
			try {
				...
				logger.sendEvent("About to processQuery:  GET_TRANSID_RESCODE",
						EventType.DEBUG, THISINSTANCE);
				result = (ArrayList<TransidRescode>) helper.processQuery(values,
						queryParms, qBuilder);
