...
public class HkdOTManageReservationXML {
  ...
  /**
   * Adds the manage reservation to the command XML stanza based on the transfer
   ...
   */
  static void addTxnBodyElement(HkdOTManageReservationTO otMngResTO, Element commandStanza) {
    ...
    // SiteNumber
    mngResStanza.addElement("SiteNumber").addText(otMngResTO.getSiteNumber().toString());

    // CommandType
    mngResStanza.addElement("CommandType").addText(otMngResTO.getCommandType());

    // ReservationIdentifier
    if (otMngResTO.getReservationCode() != null) {
      Element resId = mngResStanza.addElement("ReservationIdentifier");
      resId.addElement("ReservationCode").addText(otMngResTO.getReservationCode());
    } else if (otMngResTO.getReservationUniqueId() != null) {
      Element resId = mngResStanza.addElement("ReservationIdentifier");
      resId.addElement("ReservationUniqueId").addText(otMngResTO.getReservationUniqueId());
    }
    ...
    // PaymentInfo
    HkdOTCommandXML.addPaymentInfoStanza(mngResStanza, otMngResTO.getPaymentInfoList());

    // SellerId
    if (otMngResTO.getSellerId() != null)
      mngResStanza.addElement("SellerId").addText(otMngResTO.getSellerId().toString());
    ...
    if (otMngResTO.getAssociationInfo() != null) {
      ...
      asnInfoStanza.addElement("AssociationId").addText(otAsnInfoTO.getAssociationId().toString());

      // MemberInfo
      Element memberInfoStanza = asnInfoStanza.addElement("MemberInfo");

      if (otAsnInfoTO.getMemberId() != null) {
        memberInfoStanza.addElement("MemberId").addText(otAsnInfoTO.getMemberId().toString());
      } else {
        memberInfoStanza.addElement("MemberField").addText(otAsnInfoTO.getMemberField());
      }
      ...
      if (otAsnInfoTO.getDemographicData().size() > 0) {
        ...
        Element demoDataStanza = asnInfoStanza.addElement("DemographicData");
        ArrayList<HkdOTFieldTO> aFieldList = otAsnInfoTO.getDemographicData();
    ...
    // IATA
    if (otMngResTO.getIATA() != null)
      mngResStanza.addElement("IATA").addText(otMngResTO.getIATA());

    // TransactionNote (2.10)
    if (otMngResTO.getTransactionNote() != null) {
      mngResStanza.addElement("TransactionNote").addText(otMngResTO.getTransactionNote());
    }

    // TaxExemptCode
    if (otMngResTO.getTaxExemptCode() != null) {
      mngResStanza.addElement("TaxExemptCode").addText(otMngResTO.getTaxExemptCode());
    }

    // InTransactionAttribute (omitted)

    // ReservationNote
    if (otMngResTO.getReservationNote() != null)
      mngResStanza.addElement("ReservationNote").addText(otMngResTO.getReservationNote());
    ...
    // ShowData (as of 2.16.3, JTL)
    if (otMngResTO.getShowData() != null) {

      HkdOTShowDataTO showData = otMngResTO.getShowData();
      if ((showData.getGroupCode() != null) && (showData.getPerformanceId() != null)) {

        Element showDataStanza = mngResStanza.addElement("ShowData");
        Element showStanza = showDataStanza.addElement("Show");
        showStanza.addElement("GroupCode").addText(showData.getGroupCode());
        showStanza.addElement("PerformanceId").addText(showData.getPerformanceId());

        if (showData.getQuota() != null) {
          showStanza.addElement("Quota").addText(showData.getQuota());
        }
      }
    }
    ...
    if (otMngResTO.getGenerateEvent() != null) {
      mngResStanza.addElement("GenerateEvent").addText(otMngResTO.getGenerateEvent().toString());
    }
   ...
   * @param mngResStanza
   */
  private static void addEntitlementAccountStanza(HkdOTManageReservationTO otMngResTO, Element mngResStanza) {
    // EntitlementAccount
    if (otMngResTO.getEntitlementAccountCreationTypology() != null) {

      Element entitlementStanza = mngResStanza.addElement("EntitlementAccount");
      ...
      for (HkdOTAccountDataTO account : otMngResTO.getAccountsData()) {
        ...
        Element accountStanza = entitlementStanza.addElement("AccountData");
        accountStanza.addElement("EntitlementAccountId").addText(account.getEntitlementAccountId().toString());

        // Ignore MediaData, only use ExternalReference, or SearchFilter
        if (account.getAccountExternalReferenceType() != null && account.getAccountExternalReference() != null) {

          Element external = accountStanza.addElement("ExternalReference");
          external.addElement("AccountExternalReference").addText(account.getAccountExternalReference());
          external.addElement("AccountExternalReferenceType").addText(
              account.getAccountExternalReferenceType().toString());
          ...
          if (account.getSearchExistingMediaId() != null) {
            search.addElement("ExistingMediaId").addText(account.getSearchExistingMediaId());
          } else if (account.getSearchAccountId() != null) {
            search.addElement("AccountId").addText(account.getSearchAccountId().toString());
          } else if (account.getTicketSearchMode() != null) {
            Element ticket = search.addElement("TicketSearchMode");
            ArrayList<TicketIDType> type = account.getTicketSearchMode().getTktIdentityTypes();
            switch (type.get(0)) {

            case BARCODE:
              ticket.addElement("BarCode").addText(account.getTicketSearchMode().getBarCode());
              break;

            case TDSSN:
              Element dssn = ticket.addElement("TDSSN");
              dssn.addElement("Site").addText(account.getTicketSearchMode().getTdssnSite());
              dssn.addElement("Station").addText(account.getTicketSearchMode().getTdssnStation());
              dssn.addElement("Date").addText(
                  UtilityXML.getNexusDateFromGCal(account.getTicketSearchMode().getTdssnDate()));
              dssn.addElement("TicketId").addText(account.getTicketSearchMode().getTdssnTicketId());
              break;

            case TCOD:
              ticket.addElement("TCOD").addText(account.getTicketSearchMode().getTCOD());
              break;

            case MAGCODE:
              ticket.addElement("MagCode").addText(account.getTicketSearchMode().getMagTrack());
              break;

            case EXTERNALTICKETCODE:
              ticket.addElement("ExternalTicketCode").addText(account.getTicketSearchMode().getExternalTicketCode());
              break;
        ...
        if (account.getDemoData() != null) {
          Element demoDataStanza = accountStanza.addElement("DemographicData");
          ArrayList<HkdOTFieldTO> aFieldList = account.getDemoData().getDemoDataList();
        ...
        if (account.getMediaDataList() != null) {
          ...
          ArrayList<HkdOTMediaDataTO> aMediaList = account.getMediaDataList();

          for (HkdOTMediaDataTO mediaItem : aMediaList) {

            Element media = accountStanza.addElement("MediaData");

            media.addElement("MediaId").addText(mediaItem.getMediaId());
            media.addElement("MfrId").addText(mediaItem.getMfrId());
            media.addElement("VisualId").addText(mediaItem.getVisualId());
  ...
  /**
   * Adds the client data stanza to the XML transaction based on the transfer
   * object provided.
   * 
   * @param txnStanza
   ...
   */
  private static void addClientDataStanza(Element txnStanza, HkdOTClientDataTO cliDataTO) {
    ...
    // ClientUniqueId
    cliDataStanza.addElement("ClientUniqueId").addText(cliDataTO.getClientUniqueId().toString());

    // ClientType
    cliDataStanza.addElement("ClientType").addText(cliDataTO.getClientType());

    // ClientSubtype
    if (cliDataTO.getClientSubtype() != null)
      cliDataStanza.addElement("ClientSubtype").addText(cliDataTO.getClientSubtype().toString());

    // ClientCategory
    if (cliDataTO.getClientCategory() != null)
      cliDataStanza.addElement("ClientCategory").addText(cliDataTO.getClientCategory());

    // ClientLanguage
    if (cliDataTO.getClientLanguage() != null) {
      DecimalFormat dnf = new DecimalFormat();
      dnf.applyPattern("000");
      String clientLanguage = dnf.format(cliDataTO.getClientLanguage().longValue());
      cliDataStanza.addElement("ClientLanguage").addText(clientLanguage);
    }

    // Billing and Shipping Demographic Data (Old gateway sent all text in upper
    // case, which is
    // emulated here.) JTL
    if ((cliDataTO.getDemographicData() != null) && (cliDataTO.getDemographicData().size() > 0)) {
  ...
  /**
   * Adds the reservation data section to the XML based on the transfer object
   * provided.
   * 
   * @param mngResStanza
   *          the manage reservation stanza that needs the reservation data
   *          section.
   * @param resDataTO
   *          The Omni Ticket Reservation Data Transfer Object.
   */
  private static void addReservationDataStanza(Element mngResStanza, HkdOTReservationDataTO resDataTO) {
    ...
    // Printed
    if (resDataTO.getPrinted() != null)
      resStanza.addElement("Printed").addText(resDataTO.getPrinted().toString());

    // Encoded
    if (resDataTO.getEncoded() != null)
      resStanza.addElement("Encoded").addText(resDataTO.getEncoded().toString());

    // Validated
    if (resDataTO.getValidated() != null)
      resStanza.addElement("Validated").addText(resDataTO.getValidated().toString());

    // Deposit (omitted)

    // SalesType
    if (resDataTO.getSalesType() != null) {
      resStanza.addElement("SalesType").addText(resDataTO.getSalesType().toString());
    }

    // ResPickupDate
    if (resDataTO.getResPickupDate() != null) {
      resStanza.addElement("ResPickupDate").addText(UtilityXML.getNexusDateFromGCal(resDataTO.getResPickupDate()));
    }

    // ResPickupArea
    if (resDataTO.getResPickupArea() != null) {
      resStanza.addElement("ResPickupArea").addText(resDataTO.getResPickupArea().toString());
    }
    ...
    if (resDataTO.getResPickupType() != null) {
      DecimalFormat dnf = new DecimalFormat();
      dnf.applyPattern("000");
      String resPickupType = dnf.format(resDataTO.getResPickupType().longValue());
  ...
  /**
   * Adds the product stanza to the XML transaction based on the list of
   * transfer objects passed in.
   * 
   * @param txnStanza
   ...
   */
  private static void addProductStanza(Element txnStanza, ArrayList<HkdOTProductTO> prodList) {
    ...
    for /* each */(HkdOTProductTO aProductTO : /* in */prodList) {
      ...
      // Item
      pdtStanza.addElement("Item").addText(aProductTO.getItem().toString());
      ...
      if (aProductTO.getItemNumCode() != null)
        itemIdStanza.addElement("ItemNumCode").addText(aProductTO.getItemNumCode().toString());
      else
        itemIdStanza.addElement("ItemAlphaCode").addText(aProductTO.getItemAlphaCode());

      // Quantity
      pdtStanza.addElement("Quantity").addText(aProductTO.getQuantity().toString());

      // Validity
      if ((aProductTO.getValidity_StartDate() != null) || (aProductTO.getValidity_EndDate() != null)) {

        Element vldStanza = pdtStanza.addElement("Validity");

        if (aProductTO.getValidity_StartDate() != null) {
          String startDate = UtilityXML.getNexusDateFromGCal(aProductTO.getValidity_StartDate());
          vldStanza.addElement("StartDate").addText(startDate);
        }

        if (aProductTO.getValidity_EndDate() != null) {
          String endDate = UtilityXML.getNexusDateFromGCal(aProductTO.getValidity_EndDate());
      ...
      if (aProductTO.getPrice() != null) {
        pdtStanza.addElement("Price").addText(aProductTO.getPrice().toString());
      }
      ...
      if ((otDemoInfo != null) && (otDemoInfo.getDemoDataList().size() > 0)) {
        ...
        Element demoInfoStanza = pdtStanza.addElement("DemographicInfo");

        ArrayList<HkdOTDemographicData> otDemoDataList = otDemoInfo.getDemoDataList();

        for /* each */(HkdOTDemographicData oTDemoData : /* in */otDemoDataList) {

          Element demoDataStanza = demoInfoStanza.addElement("DemographicData");

          ArrayList<HkdOTFieldTO> oTFieldList = oTDemoData.getDemoDataList();
  ...
  /**
   * When provided with the manage reservation XML section of a parsed XML,
  ...
  static HkdOTManageReservationTO getTO(Node manageResNode) throws DTIException {
    ...
    if (errorNode == null)
      throw new DTIException(HkdOTManageReservationXML.class, DTIErrorCode.TP_INTERFACE_FAILURE,
          "Ticket provider returned XML without a ManageReservation,Error element.");
    else
      otMngResTO.setError(HkdOTCommandXML.setOTErrorTO(errorNode));
    ...
    if ((ticketInfoNodeList != null) && (ticketInfoNodeList.size() > 0))
      setOTTicketInfoTOList(otMngResTO.getTicketInfoList(), ticketInfoNodeList);

    // ProductInfo List
    List<Node> productInfoNodeList = manageResNode.selectNodes("ProductInfo");
    if (productInfoNodeList != null)
      setOTProductTOList(otMngResTO.getProductList(), productInfoNodeList);

    // PaymentInfo List
    List<Node> paymentInfoNodeList = manageResNode.selectNodes("PaymentInfo");
    if (paymentInfoNodeList != null)
      HkdOTCommandXML.setOTPaymentTOList(otMngResTO.getPaymentInfoList(), paymentInfoNodeList);

    // TransactionDSSN
    Node tranDssnNode = manageResNode.selectSingleNode("TransactionDSSN");
    if (tranDssnNode != null)
      otMngResTO.setTransactionDSSN(HkdOTCommandXML.setOTTransactionDSSNTO(tranDssnNode));

    // TransactionCOD
    Node tranCODNode = manageResNode.selectSingleNode("TransactionCOD");
    if (tranCODNode != null)
      otMngResTO.setTransactionCOD(tranCODNode.getText());

    // TransactionTime
    Node tranTimeNode = manageResNode.selectSingleNode("TransactionTime");
    if (tranTimeNode != null)
      otMngResTO.setTransactionTime(tranTimeNode.getText());
    ...
    if (taxExemptNode != null) {
      ...
      if (taxExemptValue.compareTo("0") == 0)
        otMngResTO.setTaxExempt(new Boolean(false));
      else
        otMngResTO.setTaxExempt(new Boolean(true));
    }

    // ReservationCode
    Node reservationCodeNode = manageResNode.selectSingleNode("ReservationCode");
    if (reservationCodeNode != null)
      otMngResTO.setReservationCode(reservationCodeNode.getText());

    // ReservationId
    Node reservationIdNode = manageResNode.selectSingleNode("ReservationId");
    ...
    // ReservationNote
    Node reservationNoteNode = manageResNode.selectSingleNode("ReservationNote");
    if (reservationNoteNode != null)
      otMngResTO.setReservationNote(reservationNoteNode.getText());

    // ReservationData
    Node reservationDataNode = manageResNode.selectSingleNode("ReservationData");
    if (reservationDataNode != null)
      otMngResTO.setReservationData(setOTReservationDataTO(reservationDataNode));

    // ClientData
    Node clientDataNode = manageResNode.selectSingleNode("ClientData");
    if (clientDataNode != null)
      otMngResTO.setClientData(setOTClientDataTO(clientDataNode));
   ...
   *           for any parsing errors or missing required fields.
   */
  private static void setOTTicketInfoTOList(ArrayList<HkdOTTicketInfoTO> otTktList, List<Node> ticketInfoNodeList)
      throws DTIException {
    ...
    for /* each */(Node aNode : /* in */ticketList) {
      ...
      if (ticketNode != null)
        otTktTO.setTicket(HkdOTCommandXML.setOTTicketTO(ticketNode));
   ...
   *           for any parsing errors or missing required fields.
   */
  private static void setOTProductTOList(ArrayList<HkdOTProductTO> otPdtList, List<Node> pdtNodeList)
      throws DTIException {
    ...
    for /* each */(Node aNode : /* in */pdtList) {
      ...
      if (ticketNameNode != null)
        otPdtTO.setTicketName(ticketNameNode.getText());

      // Description
      Node descNode = aNode.selectSingleNode("Description");
      if (descNode != null)
        otPdtTO.setDescription(descNode.getText());
  ...
  /**
   * Sets the reservation data transfer object based upon the parsed XML
   * provided.
   * 
   * @param resDataNode
  ...
  /**
   * Sets the demographic field list in the transfer object based upon the
   * parsed XML that is provided.
   * 
   * @param demoFieldList
   ...
   */
  @SuppressWarnings("unchecked")
  private static void setDemoFieldList(ArrayList<HkdOTFieldTO> otDemoFieldList, Node demoDataNode) throws DTIException {
