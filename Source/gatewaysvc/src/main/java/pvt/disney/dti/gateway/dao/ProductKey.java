...
/**
 * This DAO class is responsible for product database lookups such as for products, ticket types, shell types, and the product shell cross reference.
 * 
 * @author lewit019
 * 
 */
public class ProductKey {
				...
				for /* each */(TicketTO aTicket : /* in */tktListTO) {
					...
					if (aTicket.getProdCode() != null) {
						...
						if (aTicket.getProdCode().compareToIgnoreCase(
							...
							quantity = quantity + 1;
							else quantity = aTicket.getProdQty().intValue() + quantity;
						}

					}

				} // tktListTO loop
					// Make sure there are no un-printable characters; must match the xml encoding of UTF-8
	...
	public static final ArrayList<DBProductTO> getProductTicketTypes(
		...
		for /* each */(DBProductTO aDBProduct : /* in */dbProdList) {
			if (aDBProduct.getPdtid() != null) productIdSet.add(aDBProduct
					.getPdtid().toString());
		}
	...
	/**
	 * Returns the active shells in the database, based on the those provided on the order.
	 * 
	 * @param shellSet
	...
	public static final void insertProductDetail(DTITransactionTO dtiTxn) throws DTIException {
		...
		/*
		 * In the old gateway, although code was present to log reservation and create ticket information in the product detail table, it never functioned because of a comparison that was always false. When an attempt to fix this was done
		 * in the new gateway, any quantity over 9 failed because the PRODUCT_DETAIL_LOG table's quantity field is limited to a type of "number(1)".
		 * 
		 * Therefore, the reservation command was dropped (and for the same reason create ticket) because of their incompatibility with the quantity limitation.
		 */
		if ((dtiTxn.getTransactionType() == DTITransactionTO.TransactionType.CREATETICKET) || (dtiTxn
				.getTransactionType() == DTITransactionTO.TransactionType.RESERVATION)) {
			return;
		}
	...
	public static final String getProductFromTicketType(
		...
		if (result != null) {
			logger.sendEvent(
					"getProductFromTicketType found product:  " + result,
					EventType.DEBUG, THISINSTANCE, result, null);
		}
