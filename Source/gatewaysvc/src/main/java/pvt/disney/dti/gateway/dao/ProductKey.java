...
public class ProductKey {
  ...
  /** Event logger. */
  private static final EventLogger logger = EventLogger.getLogger(ProductKey.class.getCanonicalName());
  ...
  public static final ArrayList<DBProductTO> getOrderProducts(ArrayList<TicketTO> tktListTO) throws DTIException {
  ...
  public static ArrayList<DBProductTO> getOrderProducts(ArrayList<TicketTO> tktListTO, String typeCode)
      throws DTIException {
    ...
    if ((tktListTO == null) || (tktListTO.size() == 0)) {
      throw new DTIException(ProductKey.class, DTIErrorCode.INVALID_PRODUCT_CODE,
    ...
    try {
      ...
      // Run the SQL
      logger.sendEvent("About to processQuery:  GET_PRODUCTS", EventType.DEBUG, THISINSTANCE);
      result = (ArrayList<DBProductTO>) helper.processQuery(values, queryParms, qBuilder);

      // Debug
      logger.sendEvent("getOrderProducts found products.", EventType.DEBUG, THISINSTANCE, result, null);

    } catch (Exception e) {
      logger.sendEvent("Exception executing getOrderProducts: " + e.toString(), EventType.WARN, THISINSTANCE);
    ...
    if ((result != null) && (result.size() > 0)) {
      ...
      for /* each */(DBProductTO aProduct : /* in */result) {
        ...
        for /* each */(TicketTO aTicket : /* in */tktListTO) {
          ...
          // Make sure there are no un-printable characters; must match the xml
          // encoding of UTF-8
        aProduct.setQuantity(BigInteger.valueOf(quantity));
        try {
          String utf8 = new String(aProduct.getPdtDesc().getBytes("UTF-8"));
          aProduct.setPdtDesc(utf8);
        } catch (UnsupportedEncodingException e) {
          logger.sendException(EventType.EXCEPTION, ErrorCode.APPLICATION_EXCEPTION, e, THISINSTANCE);
        }
  ...
  public static final ArrayList<DBProductTO> getProductTicketTypes(ArrayList<DBProductTO> dbProdList)
    ...
    try {
      ...
      // Run the SQL
      logger.sendEvent("About to processQuery:  GET_PDT_TKT_TYPE", EventType.DEBUG, THISINSTANCE);
      resultList = (ArrayList<DBProductTO>) helper.processQuery(values, queryParms, qBuilder);

      // Debug
      logger.sendEvent("getOrderProducts found products.", EventType.DEBUG, THISINSTANCE, resultList, null);

    } catch (Exception e) {
      logger.sendEvent("Exception executing getProductTicketTypes: " + e.toString(), EventType.WARN, THISINSTANCE);
      throw new DTIException(ProductKey.class, DTIErrorCode.FAILED_DB_OPERATION_SVC,
          "Exception executing getProductTicketTypes", e);
    }

    // Enrich the existing DB Product List with the ticket type information.
    // It is more efficient to run the product query separate from the tkt map
    // query
    ...
    for /* each */(DBProductTO aDBProduct : /* in */dbProdList) {
      ...
      if (pdtTktIdMapList.get(aDBProduct.getPdtid()) != null) {
        ...
        aDBProduct.setMappedProviderTktActive(aPdtTktMap.isMappedProviderTktActive());
        aDBProduct.setMappedProviderTktNbr(aPdtTktMap.getMappedProviderTktNbr());
        aDBProduct.setMappedProviderTktName(aPdtTktMap.getMappedProviderTktName());
   ...
   */
  @SuppressWarnings("unchecked")
  public static final ArrayList<Integer> getActiveShells(HashSet<Integer> shellSet) throws DTIException {

    logger.sendEvent("Entering getActiveShells()", EventType.DEBUG, THISINSTANCE);
    ...
    try {
      ...
      // Run the SQL
      logger.sendEvent("About to processQuery:  GET_PDT_SHELL_TYPE", EventType.DEBUG, THISINSTANCE);
      result = (ArrayList<Integer>) helper.processQuery(values, queryParms, qBuilder);

      // Debug
      logger.sendEvent("getActiveShells found products.", EventType.DEBUG, THISINSTANCE, result, null);

    } catch (Exception e) {
      logger.sendEvent("Exception executing getActiveShells: " + e.toString(), EventType.WARN, THISINSTANCE);
      throw new DTIException(ProductKey.class, DTIErrorCode.FAILED_DB_OPERATION_SVC,
   ...
   */
  @SuppressWarnings("unchecked")
  public static final HashMap<String, ArrayList<Integer>> getProductShellXref(ArrayList<DBProductTO> dbProdList)
      throws DTIException {

    logger.sendEvent("Entering getProductShellXref()", EventType.DEBUG, THISINSTANCE);

    HashMap<String, ArrayList<Integer>> result = null;

    // Retrieve and validate the parameters
    if ((dbProdList == null) || (dbProdList.size() == 0)) {
      throw new DTIException(AttributeKey.class, DTIErrorCode.INVALID_PRODUCT_CODE,
    ...
    try {
      ...
      // Run the SQL
      logger.sendEvent("About to processQuery:  GET_PDT_SHELL_XREF", EventType.DEBUG, THISINSTANCE);
      result = (HashMap<String, ArrayList<Integer>>) helper.processQuery(values, queryParms, qBuilder);

      // Debug
      logger.sendEvent("getProductShellXref found products.", EventType.DEBUG, THISINSTANCE, result, null);

    } catch (Exception e) {
      logger.sendEvent("Exception executing getProductShellXref: " + e.toString(), EventType.WARN, THISINSTANCE);
      throw new DTIException(ProductKey.class, DTIErrorCode.FAILED_DB_OPERATION_SVC,
   ...
   */
  public static final void insertProductDetail(DTITransactionTO dtiTxn) throws DTIException {

    logger.sendEvent("Entering insertProductDetail()", EventType.DEBUG, THISINSTANCE);
     ...
     * functioned because of a comparison that was always false. When an attempt
     * to fix this was done in the new gateway, any quantity over 9 failed
     * because the PRODUCT_DETAIL_LOG table's quantity field is limited to a
     * type of "number(1)".
     * 
     * Therefore, the reservation command was dropped (and for the same reason
     * create ticket) because of their incompatibility with the quantity
     * limitation.
     */
    ...
    try {
      ...
      for /* each */(DBProductTO aProduct : /* in */prodList) {
        ...
        Object[] values = { productCode, productDesc, productQty, unitPrice, tax1, tax2, inboundTpId };
        DAOHelper helper = DAOHelper.getInstance(INS_PDT_DTL);
        counter += helper.processInsert(values);
      }
      logger.sendEvent("INS_PDT_DTL inserted " + counter + " rows.", EventType.DEBUG, THISINSTANCE);

    } catch (Exception e) {
      logger.sendEvent("Exception executing insertProductDetail: " + e.toString(), EventType.WARN, THISINSTANCE);
      throw new DTIException(ProductKey.class, DTIErrorCode.FAILED_DB_OPERATION_SVC,
  ...
  /**
   * Returns the product receipt information when provided with a product code.
   * 
   * @param pdtCode
   *          the product code
   ...
   */
  public static final String getProductReceipt(String pdtCode) throws DTIException {

    logger.sendEvent("Entering getProductReceipt()", EventType.DEBUG, THISINSTANCE);

    String result = null;

    // Retrieve and validate the parameters
    if (pdtCode == null) {
      throw new DTIException(ProductKey.class, DTIErrorCode.INVALID_ENTITY,
    ...
    try {
      ...
      // Run the SQL
      logger.sendEvent("About to processQuery:  GET_PDT_RECEIPT", EventType.DEBUG, THISINSTANCE);
      result = (String) helper.processQuery(values);

    } catch (Exception e) {
      logger.sendEvent("Exception executing getProductReceipt: " + e.toString(), EventType.WARN, THISINSTANCE);
      throw new DTIException(ProductKey.class, DTIErrorCode.FAILED_DB_OPERATION_SVC,
          "Exception executing getProductReceipt", e);
    }

    if (result != null) {
      logger.sendEvent("getProductReceipt found message:  " + result.toString(), EventType.DEBUG, THISINSTANCE, result,
          null);
    }
   ...
   *           on a DB or DAO problem.
   */
  public static final String getProductFromTicketType(String providerTicketType) throws DTIException {

    logger.sendEvent("Entering getProductFromTicketType()", EventType.DEBUG, THISINSTANCE);

    String result = null;

    // Retrieve and validate the parameters
    if (providerTicketType == null) {
      throw new DTIException(ProductKey.class, DTIErrorCode.MISSING_TICKET_TYPE,
    ...
    try {
      ...
      // Run the SQL
      logger.sendEvent("About to processQuery:  GET_TKT_TO_PRODUCT", EventType.DEBUG, THISINSTANCE);
      result = (String) helper.processQuery(values);

    } catch (Exception e) {
      logger.sendEvent("Exception executing getProductFromTicketType: " + e.toString(), EventType.WARN, THISINSTANCE);
      throw new DTIException(ProductKey.class, DTIErrorCode.FAILED_DB_OPERATION_SVC,
          "Exception executing getProductFromTicketType", e);
    }

    if (result != null) {
      logger.sendEvent("getProductFromTicketType found product:  " + result, EventType.DEBUG, THISINSTANCE, result,
          null);
    }
   ...
   */
  @SuppressWarnings("unchecked")
  public static ArrayList<DBProductTO> getProductCodeFromTktNbr(BigInteger tktNbr) throws DTIException {

    logger.sendEvent("Entering getProductCodeFromTktNbr()", EventType.DEBUG, THISINSTANCE);
    ...
    try {
      ...
      // Run the SQL
      logger.sendEvent("About to processQuery:  GET_PRD_FROM_TKTNBR", EventType.DEBUG, THISINSTANCE);
      result = (ArrayList<DBProductTO>) helper.processQuery(values);

    } catch (Exception e) {
      logger.sendEvent("Exception executing getProductCodeFromTktNbr: " + e.toString(), EventType.WARN, THISINSTANCE);
      throw new DTIException(ProductKey.class, DTIErrorCode.FAILED_DB_OPERATION_SVC,
          "Exception executing getProductCodeFromTktNbr", e);
    }

    if (result != null) {
      logger.sendEvent("getProductCodeFromTktNbr found product:  " + result, EventType.DEBUG, THISINSTANCE, result,
          null);
    }
