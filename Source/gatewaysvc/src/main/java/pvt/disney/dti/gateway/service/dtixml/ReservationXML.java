 ...
 * @throws DTIException 
  ...
  public static ReservationRequestTO getTO(ReservationRequest resReq) throws JAXBException, DTIException {
    ...
    if (resReq.getAPPassInfo() != null)
      resReqTO.setAPPassInfo(resReq.getAPPassInfo());
    ...
    if (resReq.getReservation() != null) {
      ...
      List<JAXBElement<?>> aReservationList = resv.getResCodeOrResNumberOrResCreateDate();
    ...
    ReservationRequest.EntitlementAccount entitlement = resReq.getEntitlementAccount();

    if (entitlement != null) {

      logger.sendEvent("ReservationXML: serach have entilement", EventType.DEBUG, THISOBJECT);

      if (entitlement.getDefaultAccount() != null) {

        if (ACCOUNT_PER_ORDER.equals(entitlement.getDefaultAccount())
            || ACCOUNT_PER_TICKET.equals(entitlement.getDefaultAccount())) {
          resReqTO.setDefaultAccount(entitlement.getDefaultAccount());
        } else {
          throw new JAXBException("Invalid <ReservationRequest><EntitlementAccount><DefaultAccount> element provided.",
        ...
        for (ReservationRequest.EntitlementAccount.SpecifiedAccount specified : 
          ...
          logger.sendEvent("ReservationXML: got an object from  specified", EventType.DEBUG, THISOBJECT);

          setTOSpecifiedAccount(specifiedTO, specified);

          logger.sendEvent("ReservationXML: got specified accountID= " + specifiedTO.getAccountItem(), EventType.DEBUG,
              THISOBJECT);
        ...
        throw new JAXBException("Invalid <ReservationRequest><EntitlementAccount> no account element provided.",
            "ReservationXML.java: getTO");
      }
    ...
    if (resReq.getExtTxnIdentifier() != null) {
      ...
      extTxnIdTO.setTxnIdentifier(extTxnId.getTxnIdentifier());
      
      if (extTxnId.isSHA1Encrypted() == null) {
        extTxnIdTO.setIsSHA1Encrypted(new Boolean("false"));
      } else {
  ...
  public static ReservationResponse getJaxb(ReservationResponseTO resRespTO, DTIErrorTO errorTO) throws JAXBException {
    ...
    if (resRespTO.getProductList() != null) {
      ...
      List<ReservationResponse.Product> productList = resResp.getProduct();
    ...
    if (resRespTO.getPaymentList() != null) {
      List<ReservationResponse.Payment> paymentList = resResp.getPayment();
   ...
   *           for any JAXB parsing issue.
   */
  private static void setJaxbPaymentList(List<ReservationResponse.Payment> paymentList,
    ...
    for /* each */(PaymentTO aPaymentTO : /* in */paymentListTO) {
      ...
      if (aPaymentTO.getPayType() == PaymentType.INSTALLMENT)
        continue; // Skip to the next pay type.
   ...
   */
  @SuppressWarnings({ "unchecked", "rawtypes" })
  private static void setJaxbTicketList(List<ReservationResponse.Ticket> ticketList, ArrayList<TicketTO> ticketListTO,
    ...
    for /* each */(TicketTO aTicketTO : /* in */ticketListTO) {
      ...
      if (typeList.size() > 0) {
        ...
        if (typeList.contains(TicketTO.TicketIdType.MAG_ID)) {
          ...
          qName = new QName("Mag");
          JAXBElement<String> tktMag = new JAXBElement(qName, tktMagTO.getClass(), tktMagTO);
        ...
        if (typeList.contains(TicketTO.TicketIdType.BARCODE_ID)) {
          ...
          JAXBElement<String> tktBar = new JAXBElement(qName, tktBarTO.getClass(), tktBarTO);
      ...
      if (aTicketTO.getTktPrice() != null) {
        ...
        JAXBElement<String> tktPrice = new JAXBElement(qName, tktPriceTO.getClass(), tktPriceTO);
      ...
      if (aTicketTO.getTktTax() != null) {
        ...
        JAXBElement<String> tktTax = new JAXBElement(qName, tktTaxTO.getClass(), tktTaxTO);
      ...
      if (statusListTO != null) {
        if (statusListTO.size() > 0) {
          for /* each */(TktStatusTO aStatusTO : /* in */statusListTO) {
            ...
            aTicket.getTktItemOrTktPartOrTktID().add(tktStatusElement);
          }
        }
      }
  ...
  private static void setTOClientData(ReservationRequest.ClientData clientData, ClientDataTO clientDataTO) {
    ...
    if (clientData.getDemoData() != null) {
      ...
      ReservationRequest.ClientData.DemoData demoData = clientData.getDemoData();
    ...
    if (clientData.getClientPaymentMethod() != null) {
      ...
      if (paymentMethod.equalsIgnoreCase(CREDIT_CARD)) {
        clientDataTO.setClientPaymentMethod(ClientDataTO.PaymentMethod.CREDITCARD);
      } else if (paymentMethod.equalsIgnoreCase(DISNEY_GIFT_CARD)) {
        clientDataTO.setClientPaymentMethod(ClientDataTO.PaymentMethod.DISNEYGIFTCARD);
      } else if (paymentMethod.equalsIgnoreCase(ORGANIZATIONAL_CHECK)) {
        clientDataTO.setClientPaymentMethod(ClientDataTO.PaymentMethod.ORGANIZATIONALCHECK);
      } else if (paymentMethod.equalsIgnoreCase(CASHIERS_CHECK)) {
        clientDataTO.setClientPaymentMethod(ClientDataTO.PaymentMethod.CASHIERSCHECK);
      } else if (paymentMethod.equalsIgnoreCase(MONEY_ORDER)) {
        clientDataTO.setClientPaymentMethod(ClientDataTO.PaymentMethod.MONEYORDER);
      } else if (paymentMethod.equalsIgnoreCase(WIRE_TRANSFER)) {
        clientDataTO.setClientPaymentMethod(ClientDataTO.PaymentMethod.WIRETRANSFER);
      } else if (paymentMethod.equalsIgnoreCase(PAY_AT_PICKUP)) {
        clientDataTO.setClientPaymentMethod(ClientDataTO.PaymentMethod.PAYATPICKUP);
      } else if (paymentMethod.equalsIgnoreCase(MASTER_ACCOUNT)) {
        clientDataTO.setClientPaymentMethod(ClientDataTO.PaymentMethod.MASTERACCOUNT);
      } else if (paymentMethod.equalsIgnoreCase(SAP_JOB_NUMBER)) {
        clientDataTO.setClientPaymentMethod(ClientDataTO.PaymentMethod.SAPJOBNUMBER);
      } else if (paymentMethod.equalsIgnoreCase(TICKET_EXCHANGE)) {
        clientDataTO.setClientPaymentMethod(ClientDataTO.PaymentMethod.TICKETEXCHANGE);
      } else {
        clientDataTO.setClientPaymentMethod(ClientDataTO.PaymentMethod.UNDEFINED);
      }
    ...
    if (clientData.getClientFulfillmentMethod() != null) {
      clientDataTO.setClientFulfillmentMethod(clientData.getClientFulfillmentMethod());
    }

    // Client Delivery Instructions
    if (clientData.getClientDeliveryInstructions() != null) {
      clientDataTO.setClientDeliveryInstructions(clientData.getClientDeliveryInstructions());
    }

    // Client Group Validity
    if (clientData.getClientGroupValidity() != null) {

      ClientGroupValidity clientGroupValidity = clientData.getClientGroupValidity();

      List<JAXBElement<XMLGregorianCalendar>> aValidityList = clientGroupValidity.getValidStartOrValidEnd();
    ...
    if (clientData.getClientSalesContact() != null) {
      clientDataTO.setClientSalesContact(clientData.getClientSalesContact());
    }
   ...
   */
  private static void setTOReservation(ReservationTO resvTO, JAXBElement<?> aReservationEntry) {
  ...
  /**
   * Given the SpecifiedAccountTO to fill, and the JAXBElement representing the
   * <SpecifiedAccount> element.
   * 
   * @param specifiedAccountTO
   * @param specifiedAccountEntry
   * @throws ParseException
   */
  private static void setTOSpecifiedAccount(SpecifiedAccountTO specifiedAccountTO,
      SpecifiedAccount specifiedAccountEntry) throws JAXBException {

    logger.sendEvent("ReservationXML: setTOSpecifiedAccount()", EventType.DEBUG, THISOBJECT);

    SpecifiedAccount specific = specifiedAccountEntry;

    // set AccountID
    if (specific.getAccountItem() != null) {
      specifiedAccountTO.setAccountItem((BigInteger) specific.getAccountItem());
    } else {
      logger.sendEvent("ReservationXML: setTOSpecifiedAccount invalid account item:"
          + specific.getAccountItem().toString(), EventType.DEBUG, THISOBJECT);
      throw new JAXBException(
    ...
    if (specific.getExistingAccount() != null) {
      ...
      logger.sendEvent(
          "ReservationXML: setTOSpecifiedAccount for:" + existing.getExistingMediaId() + ":"
              + (existing.getTktID() == null) + ":" + existing.getAccountId() + ":", EventType.DEBUG, THISOBJECT);

      if (existing.getExistingMediaId() != null) {

        specifiedAccountTO.setExistingMediaId(existing.getExistingMediaId());
        ...
        if (tktId.getMag() != null) {
          ...
          try {
            ticketId.setDssn(tktId.getTktDSSN().getTktDate(), tktId.getTktDSSN().getTktSite(), tktId.getTktDSSN()
                .getTktStation(), tktId.getTktDSSN().getTktNbr());
          } catch (ParseException parse) {
            parse.printStackTrace();
            throw (new JAXBException("ReservationXML:setTOSpecifiedAccount: TicketIDTO.setDssn", parse));
          }
        ...
        specifiedAccountTO.setExistingAccountId(existing.getAccountId());

        logger.sendEvent(
            "ReservationXML: setTOSpecifiedAccount setAccountItem:" + specifiedAccountTO.getExistingAccountId(),
            EventType.DEBUG, THISOBJECT);
      }
      ...
      specifiedAccountTO.setNewExternalReferenceType(newAccount.getExternalReferenceType());

      specifiedAccountTO.setNewExternalReferenceValue(newAccount.getExternalReferenceValue());
    ...
    if (specific.getAccountDemoData() != null) { // New as of 2.11 (JTL)
      ...
      if (acctDemoData.getDateOfBirth() != null) {
        XMLGregorianCalendar tXCal = (XMLGregorianCalendar) acctDemoData.getDateOfBirth();
        GregorianCalendar tempCalendar = UtilXML.convertFromXML(tXCal);
    ...
    if (specific.getNewMediaData() != null) {
      ...
      for (NewMediaData mediaItem : mediaList) {
        ...
        logger.sendEvent(
            "ReservationXML: setTOSpecifiedAccount for:" + mediaItem.getMediaId() + ":" + mediaItem.getMfrId() + ":"
                + mediaItem.getVisualId() + ":", EventType.DEBUG, THISOBJECT);
   ...
   */
  private static void setTOPaymentList(ArrayList<PaymentTO> paymentListTO, ReservationRequest.Payment aPayment) {
    ...
    if (payType.getCreditCard() != null) { // THIS IS A CREDIT CARD
      ...
      ReservationRequest.Payment.PayType.CreditCard creditCard = payType.getCreditCard();

      if (creditCard.getCCManual() != null) {

        ReservationRequest.Payment.PayType.CreditCard.CCManual ccManual = creditCard.getCCManual();
        ...
        if (ccManual.getCCSHA1() != null) { // As of 2.16.2, JTL
          creditCardTO.setCcSha1(ccManual.getCCSHA1());
        }
        if (ccManual.getCCSubCode() != null) { // As of 2.16.2, JTL
          creditCardTO.setCcSubCode(ccManual.getCCSubCode());
        }
        if (ccManual.getCCAuthCode() != null) { // As of 2.16.2
          creditCardTO.setCcAuthCode(ccManual.getCCAuthCode());
        }
        ...
        ReservationRequest.Payment.PayType.CreditCard.CCSwipe ccSwipe = creditCard.getCCSwipe();
      ...
      if (payType.getGiftCard().getGCManual() != null) {
        ReservationRequest.Payment.PayType.GiftCard.GCManual gcManual = payType.getGiftCard().getGCManual();
        ...
        ReservationRequest.Payment.PayType.GiftCard.GCSwipe gcSwipe = payType.getGiftCard().getGCSwipe();
      ...
      ReservationRequest.Payment.PayType.Installment installment = payType.getInstallment();
 ...
 * @throws DTIException 
  ...
  private static void setTOTicketList(ArrayList<TicketTO> ticketListTO, ReservationRequest.Ticket aTicket) throws DTIException {
    ...
    if (aTicket.getTktValidity() != null) {
      ...
      List<JAXBElement<XMLGregorianCalendar>> aValidityList = aTicket.getTktValidity().getValidStartOrValidEnd();
    ...
    if (aTicket.getTktAssignment() != null) {
      for /* each */(TktAssignment assignment : /* in */aTicket.getTktAssignment()) {
