...
public class GWOrderUpgradeEntitlementXML {
	...
	private static EventLogger eventLogger = EventLogger.getLogger(GWOrderXML.class);
	 ...
	 *             the DTI exception
	 */
	public static void addOrderElement(GWOrdersRqstTO orderReqTO, Element bodyElement) throws DTIException {
		...
		if (!orderIter.hasNext()) {
			throw new DTIException(GWOrderXML.class, DTIErrorCode.INVALID_MSG_CONTENT,
		...
		while (orderIter.hasNext()) {
			...
			// OrderStatus
			if (orderTO.getOrderStatus() != null && !orderTO.getOrderStatus().equalsIgnoreCase("null")
					&& orderTO.getOrderStatus().length() > 0)
				orderElement.addElement("OrderStatus").addText(orderTO.getOrderStatus());
			...
			if (orderTO.getPaymntContractList().size() > 0) {
				Element pmntCntrctsElement = orderElement.addElement("PaymentContracts");
			...
			// OrderReference
			if (orderTO.getOrderReference() != null && !orderTO.getOrderReference().equalsIgnoreCase("null")
					&& orderTO.getOrderReference().length() > 0)
				orderElement.addElement("OrderReference").addText(orderTO.getOrderReference());
			...
			// PCI Control -DO NOT REMOVE!!! (unless you removed the debug
			// statement
			String cleanXML = PCIControl.overwritePciDataInXML(bodyElement.asXML());
			// END PCI Controls.
			eventLogger.sendEvent("GWOrderXML: XML Body Element: '" + cleanXML, EventType.DEBUG, Object.class);
	...
	private static void addOrderLines(GWOrderTO orderTO, Element orderElement) throws DTIException {
		...
		while (lineIter.hasNext()) {
			...
			if (detailType.equals(TransformConstants.GW_ORDERS_TICKET_ORDER_LINE_ITEM)) {
				...
				// null check for billing ZIP, cause gift cards and other
				// payments
				// don't have it
				...
				// null check for expirationDate, cause gift cards and other
				// payments
				// don't have it
				...
				// null check for CVN, cause gift cards and other payments don't
				// have
				// it
				...
				// Are there demographics (there should be only one per line
				// item,
			...
			// for upgrade TO Do
			else if (detailType.equals(TransformConstants.GW_ORDERS_UPGRADE_ORDER_LINE_ITEM)) {
				...
				// TaxAmount need to get the details for tax , need to verify
				// this TO DO
				Double taxAmount = Double.parseDouble(lineTO.getTotal()) - Double.parseDouble(lineTO.getAmount());
				orderLineElement.addElement("TaxAmount").addText(taxAmount.toString());

				// Need to verify this TO DO
				orderLineElement.addElement("DiscountAmount").addText("0.0");
	...
	private static void addShipToContact(GWOrderTO orderTO, Element orderElement) {
		...
		if (orderTO.getShipToContact() != null) {
			...
			if (firstName != null && firstName.length() > 0 && !(firstName.equals("null"))) {
				shipContact.addElement("FirstName").addText(firstName);
			}
			if (lastName != null && lastName.length() > 0 && !(lastName.equals("null"))) {
				shipContact.addElement("LastName").addText(lastName);
			}

			if (street1 != null && street1.length() > 0 && !(street1.equals("null"))) {
				shipContact.addElement("Street1").addText(street1);
			}

			if (street2 != null && street2.length() > 0 && !(street2.equals("null"))) {
				shipContact.addElement("Street2").addText(street2);
			}

			if (street3 != null && street3.length() > 0 && !(street3.equals("null"))) {
	...
	public static void setRespBodyTO(GWBodyTO gwBodyTO, Element bodyElement) throws DTIException {
		...
		for (Iterator<org.dom4j.Element> bodyIterator = bodyElement.elementIterator(); bodyIterator.hasNext();) {
			...
			if (element.getName().equalsIgnoreCase("PaymentContracts")) {
				...
				for (Iterator<org.dom4j.Element> contractRespIter = element.elementIterator(); contractRespIter
						.hasNext();) {
					Element childElement = contractRespIter.next();

					if (childElement.getName().equalsIgnoreCase("PaymentContractID")) {
						if (gwOrdersRespTO.getPaymentContractID() == null) {
							gwOrdersRespTO.setPaymentContractID(childElement.getText());
						}
					}
				}
			}
			...
			if (element.getName().compareTo("CreateTransactionResponse") == 0) {
				...
				// Locate the Products element
				for (Iterator<org.dom4j.Element> crtTxnRespIter = element.elementIterator(); crtTxnRespIter
						.hasNext();) {
		...
		if ((gwBodyTO.getOrdersRespTO()) == null && (gwBodyTO.getOrdersErrorsTO() == null)) {
	...
	public static void setProducts(Element productsElement, GWOrdersRespTO gwOrdersRespTO) {
		...
		for (Iterator<org.dom4j.Element> productListIter = productsElement.elementIterator(); productListIter
				.hasNext();) {

			Element productElement = productListIter.next();

			// Validate that this element is marked (attributed) as a
			// Type="Ticket"
			boolean isATicket = false;
			for (Iterator<org.dom4j.Attribute> attrIter = productElement.attributeIterator(); attrIter.hasNext();) {
		...
		eventLogger.sendEvent("GWOrderXML.setProducts() found this number of tickets: '" + gwTicketListTO.size(),
	...
	private static GWStatusTO setStatusTO(Element bodyElement) throws DTIException {
		...
		org.dom4j.XPath statusCodeSelector = DocumentHelper.createXPath("//Status");
		...
		// we need to manipulate status code and text here since order status
		// and
		...
		if (statusCode.equals(PAYMENT_STATUS_APPROVED)) {
			...
			org.dom4j.XPath errorCodeSelector = DocumentHelper.createXPath("//OrderStatusError/ErrorCode");
			statusCode = errorCodeSelector.selectSingleNode(bodyElement).getText();

			org.dom4j.XPath errorTextSelector = DocumentHelper.createXPath("//OrderStatusError/ErrorText");
			statusText = errorTextSelector.selectSingleNode(bodyElement).getText();
		}
	...
	/**
	 * 
	 * Unmarshall the errors element into the gateway orders errors transfer
	 * object.
	 * 
	 * @param errorsElement
	...
	private static GWOrdersErrorsTO setErrorsTO(Element errorsElement) throws DTIException {
		...
		org.dom4j.XPath statusCodeSelector = DocumentHelper.createXPath("//OrderStatusError/ErrorCode");
		String statusCode = statusCodeSelector.selectSingleNode(errorsElement).getText();

		org.dom4j.XPath statusTextSelector = DocumentHelper.createXPath("//OrderStatusError/ErrorText");
		String statusText = statusTextSelector.selectSingleNode(errorsElement).getText();
