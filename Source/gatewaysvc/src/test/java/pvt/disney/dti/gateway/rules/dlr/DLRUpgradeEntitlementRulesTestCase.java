...
public class DLRUpgradeEntitlementRulesTestCase extends DTIMockUtil {
	...
	public void testTransformRequest() {
		...
		tpLookupTOListIn.get(0).setLookupDesc("SalesProgram");
		tpLookupTOListIn.get(0).setLookupType(TPLookupTO.TPLookupType.INSTALLMENT);
		tpLookupTOListIn.get(1).setLookupDesc("SalesProgram");
		tpLookupTOListIn.get(1).setLookupType(TPLookupTO.TPLookupType.INSTALLMENT);
		tpLookupTOListIn.remove(2);
		dtiTxn.setTpLookupTOList(tpLookupTOListIn);
		
		((UpgradeEntitlementRequestTO) dtiTxn.getRequest().getCommandBody()).setInstallmentRequest(true);
		try {
			xmlString = DLRUpgradeEntitlementRules.transformRequest(dtiTxn);
			assertNotNull(xmlString);
		} catch (DTIException dtie) {
