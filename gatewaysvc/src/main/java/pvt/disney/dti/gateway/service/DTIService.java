...
<<<<<<< HEAD
   ...
   */
  public static String submitRequest(String request, EntityTO entityTO, Integer transIdITS,
      Integer tpRefNumber) {
    ...
    response = dtiServ.makeServiceRequest(request, entityTO, transIdITS, tpRefNumber);
   ...
   * @return XML response of type string.
   */
  private String makeServiceRequest(String requestXML, EntityTO entityTO, Integer transIdITS,
      Integer tpRefNumber) {
    ...
    logger.sendEvent("DTIService determined type of request is " + requestType, EventType.INFO,
        this);
    ...
    try {
      ...
      logger.sendEvent("Checked Contingency Actions Logic Module (CALM).", EventType.DEBUG, this);
      ...
      logger
          .sendEvent(
              "DTI ERROR CODE: " + dtie.getDtiErrorCode().getErrorCode() + " - "
                  + dtie.getLogMessage(), EventType.WARN, this);
      logger.sendEvent("DTI application class: " + dtie.getClassName().toString(), EventType.WARN,
          this);
      ...
      logger.sendEvent("Warning: CALM Override for provider " + dtiTxn.getTpiCode(),
          EventType.WARN, this);
    ...
    try {
      ...
      logger.sendEvent("Exception parsing resp object:  " + je.toString(), EventType.EXCEPTION,
          this);
      logger.sendEvent("Error code was: " + je.getErrorCode() + ":  " + je.getLocalizedMessage(),
          EventType.EXCEPTION, this);
      return generateGenericError(DTIErrorCode.TP_INTERFACE_FAILURE, requestXML);

    } catch (DTIException qe) {
      logger.sendEvent("Exception retrieving resp xml:  " + qe.toString(), EventType.EXCEPTION,
          this);
      return generateGenericError(DTIErrorCode.TP_INTERFACE_FAILURE, requestXML);
  ...
  public static String generateGenericError(DTIErrorCode dtiError, String requestXML) {
    ...
    resp.append("</TransmitTime><TktBroker>" + tktBroker
        + "</TktBroker><CommandCount>0</CommandCount>");

    resp.append("<PayloadError><HdrErrorCode>" + errorTO.getErrorCode().toString()
        + "</HdrErrorCode>");
...
=======
	...
	public static String submitRequest(String request, EntityTO entityTO,
			Integer transIdITS, Integer tpRefNumber) {
	...
	private String makeServiceRequest(String requestXML, EntityTO entityTO,
		...
		try {
			...
			calmRules.checkContingencyActionsLogicModule(dtiTxn);
			logger.sendEvent(
					"Checked Contingency Actions Logic Module (CALM).",
