...
<<<<<<< HEAD
  ...
  /** The event logger. */
  private static final EventLogger logger = EventLogger.getLogger(LookupKey.class
      .getCanonicalName());
  ...
  /**
   * The constant value representing the get ticket provider command lookup
   * query.
   */
  private static final String GET_TP_CMD_LOOKUP = "GET_TP_CMD_LOOKUP";

  /**
   * The constant value representing the get gateway eGalaxy ticket provider
   * lookup
   */
  ...
  /**
   * The constant indicating a ManualMailOrder sales type. Treated like a
   * Pre-sale
   */
  private static final String RES_SALES_TYPE_MANUALMAILORDER = "ManualMailOrder";

  /**
   * The constant indicating an ElectronicEntitlement sales type. Treated like
   * mail order.
   */
  ...
  public static final ArrayList<PaymentLookupTO> getPaymentLookup(String commandCode,
      String ticketProviderId, long entityId, String actor) throws DTIException {

    logger.sendEvent("Entering getPaymentLookup()", EventType.DEBUG, THISINSTANCE);
    ...
    if ((commandCode == null) || (ticketProviderId == null) || (actor == null) || (entityId <= 0)) {
      throw new DTIException(LookupKey.class, DTIErrorCode.INVALID_ENTITY,
    ...
    try {
      ...
      // Run the SQL
      logger.sendEvent("About to processQuery:  GET_PMT_LOOKUP", EventType.DEBUG, THISINSTANCE);
      result = (ArrayList<PaymentLookupTO>) helper.processQuery(values);

      // Debug
      logger.sendEvent("getPaymentLookup with Actor found attributes.", EventType.DEBUG,
          THISINSTANCE, result, null);

    } catch (Exception e) {
      logger.sendEvent("Exception executing getPaymentLookup: " + e.toString(), EventType.WARN,
          THISINSTANCE);
   ...
   * @throws DTIException
   */
  public static final ArrayList<TPLookupTO> getTPCommandLookup(String tpiCode,
      DTITransactionTO.TransactionType txnType) throws DTIException {

    return getTPCommandLookup(tpiCode, txnType, "NONE", "NONE", "NONE", "NONE");
  ...
  @SuppressWarnings("unchecked")
  public static final ArrayList<TPLookupTO> getTPCommandLookup(String tpiCode,
      DTITransactionTO.TransactionType txnType, String language, String clientType,
      String resPickupArea, String resSalesType) throws DTIException {

    logger.sendEvent("Entering getTPCommandLookup()", EventType.DEBUG, THISINSTANCE);
    ...
    String pickupType;
    if (RES_SALES_TYPE_MANUALMAILORDER.compareToIgnoreCase(resSalesType) == 0
        || resSalesType.compareToIgnoreCase(RES_SALES_TYPE_PRESALE) == 0) {
      pickupType = RES_PICKUP_TYPE_WILLCALL;
    } else if (RES_SALES_TYPE_ELECTRONICENTL.compareToIgnoreCase(resSalesType) == 0) {
    ...
    // Retrieve and validate the parameters
    Object[] values = { tpiCode, commandCode, language, clientType, resSalesType, pickupType,
        resPickupArea };
    ...
    try {
      ...
      // Run the SQL
      logger.sendEvent("About to processQuery:  GET_TP_CMD_LOOKUP", EventType.DEBUG, THISINSTANCE);
      result = (ArrayList<TPLookupTO>) helper.processQuery(values);

      // Debug
      logger.sendEvent("getTPCommandLookup with Actor found attributes.", EventType.DEBUG,
          THISINSTANCE, result, null);

    } catch (Exception e) {
      logger.sendEvent("Exception executing getTPCommandLookup: " + e.toString(), EventType.WARN,
          THISINSTANCE);
      throw new DTIException(LookupKey.class, DTIErrorCode.FAILED_DB_OPERATION_SVC,
          "Exception executing getPaymentLookup", e);
    }
   ...
   * Returns an array of TP Command Lookup values
   * 
   * @param tpiCode The ticket provider interface code.
   * @param txnType The transaction type being attempted.
   * @return an array of TP Command Lookup values
   * @throws DTIException on a DB or DAO error.
   */
  public static final String getSimpleTPLookup(Integer ticketProviderId, String lookup_key,
      String lookup_type) throws DTIException {

    logger.sendEvent("Entering getSimpleTPLookup()", EventType.DEBUG, THISINSTANCE);

    String result = null;

    // Retrieve and validate the parameters
    if ((lookup_key == null) || (lookup_type == null)) {
      throw new DTIException(LookupKey.class, DTIErrorCode.UNDEFINED_FAILURE,
    ...
    try {
      ...
      // Run the SQL
      logger
          .sendEvent("About to processQuery:  GET_SIMPLE_TPLOOKUP", EventType.DEBUG, THISINSTANCE);
      result = (String) helper.processQuery(values);

      // Debug
      logger.sendEvent("getSimpleTPLookup found results", EventType.DEBUG, THISINSTANCE, result,
          null);

    } catch (Exception e) {
      logger.sendEvent("Exception executing getSimpleTPLookup: " + e.toString(), EventType.WARN,
          THISINSTANCE);
      throw new DTIException(LookupKey.class, DTIErrorCode.FAILED_DB_OPERATION_SVC,
   ...
   *           on a DB or DAO error.
   */
  public static final Integer getVoidLookup(String statusValue, String ticketProviderId)
      throws DTIException {

    logger.sendEvent("Entering getVoidLookup()", EventType.DEBUG, THISINSTANCE);

    Integer result = null;

    // Retrieve and validate the parameters
    if ((statusValue == null) || (ticketProviderId == null)) {
      throw new DTIException(LookupKey.class, DTIErrorCode.INVALID_MSG_CONTENT,
    ...
    try {
      ...
      // Run the SQL
      logger.sendEvent("About to processQuery:  GET_VOID_CODE", EventType.DEBUG, THISINSTANCE);
      result = (Integer) helper.processQuery(values);

      // Debug
      logger.sendEvent("getVoidLookup found results.", EventType.DEBUG, THISINSTANCE, result, null);

    } catch (Exception e) {
      logger.sendEvent("Exception executing getVoidLookup: " + e.toString(), EventType.WARN,
          THISINSTANCE);
      throw new DTIException(LookupKey.class, DTIErrorCode.FAILED_DB_OPERATION_SVC,
   ...
   */
  @SuppressWarnings("unchecked")
  public static final ArrayList<TPLookupTO> getGWTPCommandLookup(String tpiCode,
      DTITransactionTO.TransactionType txnType, String shipMethod, String shipDetail)
      throws DTIException {

    logger.sendEvent("Entering getGWTPCommandLookup()", EventType.DEBUG, THISINSTANCE);
    ...
    try {
      ...
      // Run the SQL
      logger.sendEvent("About to processQuery:  GET_GW_TP_CMD_LOOKUP", EventType.DEBUG,
          THISINSTANCE);
      result = (ArrayList<TPLookupTO>) helper.processQuery(values);

      // Debug
      logger.sendEvent("getGWTPCommandLookup with Actor found attributes.", EventType.DEBUG,
          THISINSTANCE, result, null);

    } catch (Exception e) {
      logger.sendEvent("Exception executing getGWTPCommandLookup: " + e.toString(), EventType.WARN,
          THISINSTANCE);
      throw new DTIException(LookupKey.class, DTIErrorCode.FAILED_DB_OPERATION_SVC,
          "Exception executing getPaymentLookup", e);
    }
