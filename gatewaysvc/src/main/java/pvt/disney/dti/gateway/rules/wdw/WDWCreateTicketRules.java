...
<<<<<<< HEAD
  ...
  private static final EventLogger logger = EventLogger.getLogger(WDWCreateTicketRules.class.getCanonicalName());
  ...
  static String transformRequest(DTITransactionTO dtiTxn) throws DTIException {
    ...
    // === Command Level ===
    OTCommandTO atsCommand = new OTCommandTO(OTCommandTO.OTTransactionType.CREATETRANSACTION);
    atsCommand.setXmlSchemaInstance(WDWBusinessRules.XML_SCHEMA_INSTANCE);
    atsCommand.setNoNamespaceSchemaLocation(NO_NAMESPACE_SCHEMA_LOCATION);

    // === Header Level ===
    OTHeaderTO hdr = WDWBusinessRules.transformOTHeader(dtiTxn, REQUEST_TYPE_CREATE, REQUEST_SUBTYPE_CREATE_TXN);
    atsCommand.setHeaderTO(hdr);
    ...
    if (anAttributeTO == null) {
      ...
      otCrtTxnTO.setSiteNumber(Integer.parseInt(anAttributeTO.getAttrValue()));
    }
    // Product
    ArrayList<OTProductTO> otProdList = otCrtTxnTO.getProductList();
    ArrayList<TicketTO> dtiTktList = dtiCreateTktReq.getTktList();
    HashMap<String, DBProductTO> dbProdMap = dtiTxn.getDbProdMap();
    boolean productsAssignAccounts = createOTProducts(otProdList, dtiTktList, dbProdMap);
    ...
    if (ACCOUNT_PER_ORDER.equals(dtiCreateTktReq.getDefaultAccount())) {
      ...
      otCrtTxnTO.setEntitlementAccountCreationTypology(PER_TICKET_ACCOUNT);

    } else {

      if (dtiCreateTktReq.getSpecifiedAccounts() != null && dtiCreateTktReq.getSpecifiedAccounts().size() > 0) {

        // ENTITLEMENTACCOUNTID always requires a tag to be present in
        // the product node Per Andrea Martinotti 10/2/2012
        if (productsAssignAccounts) {

          otCrtTxnTO.setEntitlementAccountCreationTypology(ENTITLEMENT_ACCOUNT_ID);
        ...
        transformSpecifiedAccount(dtiCreateTktReq, otCrtTxnTO.getAccountsData());
    ...
    // InTransactionAttributes
    if ((otCrtTxnTO.getInTxnAttrList() != null) && (otCrtTxnTO.getInTxnAttrList().size() > 0)) {
      ArrayList<OTInTransactionAttributeTO> inTxnAttrList = otCrtTxnTO.getInTxnAttrList();
    ...
    // As of 2.12, override with Audit Notification if available. 
    if (dtiCreateTktReq.getAuditNotation() == null) {
      otCrtTxnTO.setTransactionNote(dtiRequest.getPayloadHeader().getPayloadID());
    } else {
    	otCrtTxnTO.setTransactionNote(dtiCreateTktReq.getAuditNotation());
    }
   ...
   *           if unable to get the EligibilityAssocId
   */
  private static OTAssociationInfoTO setAssociationInfo(CreateTicketRequestTO dtiResReq) throws DTIException {
    OTAssociationInfoTO otAssocInfo = new OTAssociationInfoTO();
  ...
  /**
   * Creates the Omni Ticket product list from the DTI Product List and Database
   * Product Map provided.
   * 
   * @param otProdList
   *          The Omni Ticket Product List.
   * @param dtiTktList
   ...
   *          The DTI Database Product Map.
   */
  private static boolean createOTProducts(ArrayList<OTProductTO> otProdList, ArrayList<TicketTO> dtiTktList,
      HashMap<String, DBProductTO> dbProdMap) {
    ...
    for /* each */(TicketTO aDtiTicket : /* in */dtiTktList) {
      ...
      // Validity Dates
      // StartDate
      if (dbProduct.isValidityDateInfoRequired() && aDtiTicket.getTktValidityValidStart() != null) {
        otProduct.setValidity_StartDate(aDtiTicket.getTktValidityValidStart());
      }

      // EndDate
      if (dbProduct.isValidityDateInfoRequired() && aDtiTicket.getTktValidityValidEnd() != null) {
      ...
      // EntitlementAccountId, list of id, one for each non-consumable
      // ticket of this product tied to that account
      if ((aDtiTicket.getTicketAssignmets().size() > 0) && (dbProduct.isConsumable() == false)) {
  ...
  /**
   * Transforms a create ticket response string from the WDW provider and
  ...
  static void transformResponseBody(DTITransactionTO dtiTxn, OTCommandTO otCmdTO, DTIResponseTO dtiRespTO)
    ...
    if ((otTicketList != null) && (otTicketList.size() > 0)) {
      ...
      for /* each */(OTTicketInfoTO otTicketInfo : /* in */otTicketList) {
        ...
        TicketTransactionTO dtiTktTranTO = new TicketTransactionTO();
        dtiTktTranTO.setTktProvider(WDW_PROVIDER);
        dtiTktTranTO.setDssnDate(otCrtTxnTO.getTransactionDSSN().getDate());
        dtiTktTranTO.setDssnSite(otCrtTxnTO.getTransactionDSSN().getSite());
        dtiTktTranTO.setDssnStation(otCrtTxnTO.getTransactionDSSN().getStation());
        dtiTktTranTO.setDssnNumber(otCrtTxnTO.getTransactionDSSN().getTransactionId().toString());
        dtiTicketTO.setTktTran(dtiTktTranTO);
   ...
   * @throws DTIException
   */
  private static void transformSpecifiedAccount(CreateTicketRequestTO dtiResReq, ArrayList<OTAccountDataTO> otAccounts)
      throws DTIException {
    ...
    for (SpecifiedAccountTO dtiAccount : dtiResReq.getSpecifiedAccounts()) {
      ...
      if ((dtiAccount.getNewExternalReferenceType() != null) && 
          (dtiAccount.getNewExternalReferenceValue() != null)) {

        logger.sendEvent("WDWCreateTicketRules: search by External Reference type", EventType.DEBUG,
            WDWCreateTicketRules.class);
        ...
        if (otType == null) {
          // ERROR shouldn't happen
          throw new DTIException(WDWCreateTicketRules.class, DTIErrorCode.INVALID_MSG_CONTENT,
              "Invalid ExternalReferenceType submited.");
        } else {
          otAccountData.setAccountExternalReferenceType(otType);
        }
        ...
        // search for existing account
        logger.sendEvent(
            "WDWCreateTicketRules: Adding Existing account",EventType.DEBUG, THISOBJECT);

        if (dtiAccount.getExistingMediaId() != null) {

          otAccountData.setSearchExistingMediaId(dtiAccount.getExistingMediaId());
          logger.sendEvent(
              "Just added existing media ID: " + dtiAccount.getExistingMediaId() + " to otAccountData.",
              EventType.DEBUG, THISOBJECT);
          ...
          logger.sendEvent("WDWCreateTicketRules: Got existingAccountId:" + dtiAccount.getExistingAccountId(),
              EventType.DEBUG, THISOBJECT);
  ...
  public static void applyWDWCreateTicketRules(DTITransactionTO dtiTxn) throws DTIException {
    ...
    // Validate the electronic entitlement account section, if present (as of 2.10)
    if (crtTktReq.getSpecifiedAccounts() != null) {
      ElectronicEntitlementRules.validateSpecifiedAccounts(crtTktReq.getSpecifiedAccounts());
    }
