...
<<<<<<< HEAD
   ...
   * Enforces the following rules: <BR>
   * 1. Does the number and types of payments exceed provider limitations. 2.
   * Does the order include illegal payment types for a provider.
   * 
   * @param payListTO
   *          the payment transfer object list
   * @param tpiCode
   ...
   * RULE: If Installment Payment type is present, only one additional form of 
   * payment is allowed and it must be the same credit card as installment. (as of 2.16.1 JTL)
   * @param dtiTxn
   * @param dtiResReq
   * @param tpLookups
   * @throws DTIException
  ...
  public static void validateResInstallDownpayment(DTITransactionTO dtiTxn, ArrayList<TPLookupTO> tpLookups)
    ...
    // Preserve down-payment amount for later use in the out-bound transaction to eGalaxy.
...
=======
	 ...
	 * Enforces the following rules: <BR>
	 * 
	 * RULE: If a payment section is provided, payments must match up to the cost of items purchased. <BR>
	 * RULE: Validate that all credit cards are between (inclusive) 12 and 19 digits in length. <BR>
	 * RULE: Ensure that gift card numbers are being received and transmitted as numeric only (as of 2.12). <BR>
	 * 
	 * @param tktListTO
	 *            the tkt transfer object list
	 * @param payListTO
	 ...
	 *             should any rule fail.
	 */
	public static BigDecimal validatePaymentsOnOrder(
			ArrayList<TicketTO> tktListTO, ArrayList<PaymentTO> payListTO,
			EntityTO entityTO) throws DTIException {
		...
		for /* each */(TicketTO aTicketTO : /* in */tktListTO) {
			...
			if (aTicketTO.getTicketTypes().size() > 0) {
				totalProductCost = totalProductCost.add(aTicketTO
						.getProdPrice());
			}
			else {
				BigDecimal prodCost = aTicketTO.getProdPrice();
				BigInteger prodQty = aTicketTO.getProdQty();
				BigDecimal prodTotal = prodCost
						.multiply(new BigDecimal(prodQty));
				totalProductCost = totalProductCost.add(prodTotal);
			}
		...
		if ((payListTO == null) || (payListTO.size() == 0)) {
			...
			for /* each */(PaymentTO aPaymentTO : /* in */payListTO) {
				...
				if ((aPaymentTO.getPayAmount() != null) && (aPaymentTO
						.getPayAmount().floatValue() > 0.0)) {
					totalPaymentsMade = totalPaymentsMade.add(aPaymentTO
							.getPayAmount());
				}
				...
				if (aPaymentTO.getPayType() == PaymentTO.PaymentType.CREDITCARD) {
					...
					if (aCreditCard.getCcManualOrSwipe() == CreditCardTO.CreditCardType.CCMANUAL) {
						...
						if ((aCreditCard.getCcNbr().length() > MAX_MANUAL_CCGC_LENGTH) || (aCreditCard
								.getCcNbr().length() < MIN_MANUAL_CCGC_LENGTH)) {

							throw new DTIException(
									PaymentRules.class,
									DTIErrorCode.INVALID_PAYMENT_TYPE,
									"Invalid credit card account number length on PayItem " + aPaymentTO
											.getPayItem().toString() + " expected to be between " + MIN_MANUAL_CCGC_LENGTH + " and " + MAX_MANUAL_CCGC_LENGTH + ", but length was " + aCreditCard
											.getCcNbr().length() + ".");
						}
					}
				...
				if (aPaymentTO.getPayType() == PaymentTO.PaymentType.GIFTCARD) {
					...
					if (aGiftCard.getGcManualOrSwipe() == GiftCardTO.GiftCardType.GCMANUAL) {
						...
						if ((aGiftCard.getGcNbr().length() > MAX_MANUAL_CCGC_LENGTH) || (aGiftCard
								.getGcNbr().length() < MIN_MANUAL_CCGC_LENGTH)) {

							throw new DTIException(
									PaymentRules.class,
									DTIErrorCode.INVALID_PAYMENT_TYPE,
									"Invalid gift card account number length on PayItem " + aPaymentTO
											.getPayItem().toString() + " expected to be between " + MIN_MANUAL_CCGC_LENGTH + " and " + MAX_MANUAL_CCGC_LENGTH + ", but length was " + aGiftCard
											.getGcNbr().length() + ".");
						}
					}

					if (!CustomDataTypeConverter
							.isNumeric(aGiftCard.getGcNbr())) {
						throw new DTIException(
								PaymentRules.class,
								DTIErrorCode.INVALID_PAYMENT_TYPE,
								"Invalid gift card account number (non-numeric characters detected) on PayItem " + aPaymentTO
										.getPayItem().toString() + ".");
					}
				...
				if (aPaymentTO.getPayType() == PaymentTO.PaymentType.INSTALLMENT) {
					...
					InstallmentCreditCardTO aCreditCard = installTO
							.getCreditCard();
					if (installTO.getCreditCard().getCcManualOrSwipe() == CreditCardType.CCMANUAL) {
				...
				if (payment != null) {
					if (payment.compareTo(new BigDecimal("0.0")) == 0) {
						throw new DTIException(PaymentRules.class,
			...
			// If payments and product cost don't match, throw exception.
			if (totalProductCost.compareTo(totalPaymentsMade) != 0) throw new DTIException(
					PaymentRules.class,
					DTIErrorCode.INVALID_PAYMENT_AMOUNT,
					"Product cost total of " + totalProductCost.toString() + " did not equal total payments made of " + totalPaymentsMade
							.toString());
		}
	 ...
	 *             should any rule fail.
	 */
	public static void validatePaymentsPresent(ArrayList<PaymentTO> payListTO,
			EntityTO entityTO) throws DTIException {
	 ...
	 * Enforces the following rules: <BR>
	 * 
	 * 1. If a payment section is provided, payments must match up to the cost of upgrades purchased. 2. Validate that all credit cards are between (inclusive) 12 and 19 digits in length.
	 * 
	 * @param tktListTO
	 *            the tkt transfer object list
	 * @param payListTO
	 ...
	 *             should any rule fail.
	 */
	public static void validatePaymentsOnUpgrade(ArrayList<TicketTO> tktListTO,
			ArrayList<PaymentTO> payListTO, EntityTO entityTO) throws DTIException {
		...
		if ((payListTO == null) || (payListTO.size() == 0)) {
			...
			if (totalProductCost.floatValue() > 0.0) {
				throw new DTIException(
						PaymentRules.class,
						DTIErrorCode.INVALID_PAYMENT_TYPE,
						"Product cost total of " + totalProductCost.toString() + " did not equal total payments made of 0.0.");
			}
	...
	/**
	 * Validate payment composition. Limits are placed upon the number of gift cards or credit cards that can be used in combination, but the number of vouchers is not limited beyond the maximum number of payments. Validates the maximum
	 * number of payments allowed. <BR>
	 * Enforces the following rules: <BR>
	 * 1. Does the number and types of payments exceed provider limitations. 2. Does the order include illegal payment types for a provider.
	 * 
	 * @param payListTO
	 *            the payment transfer object list
	 * @param tpiCode
	 ...
	 *             should any rule fail.
	 */
	public static void validatePaymentComposition(
			ArrayList<PaymentTO> payListTO, String tpiCode) throws DTIException {

		if (payListTO == null) return;
		...
		if ((payListTO != null) && (payListTO.size() > 0)) {
			...
			if (numberOfPayments > maxNumberOfPayments) throw new DTIException(
					PaymentRules.class,
					DTIErrorCode.INVALID_PAYMENT_COUNT,
					"Number of payments provided " + numberOfPayments + " exceeded " + tpiCode + " maximum of " + maxNumberOfPayments);
		...
		for /* each */(PaymentTO aPaymentTO : /* in */payListTO) {
			...
			if ((aPaymentTO.getPayType() == PaymentTO.PaymentType.CREDITCARD) || (aPaymentTO
					.getPayType() == PaymentTO.PaymentType.GIFTCARD)) {
				numberOfCCGC++;

				if (aPaymentTO.getPayAmount() == null) throw new DTIException(
						PaymentRules.class,
						DTIErrorCode.INVALID_PAYMENT_AMOUNT,
						"Gift Cards and Credit Cards must have a payment amount (PayItem: " + aPaymentTO
								.getPayItem().intValue() + ").");
			}
		}

		if (numberOfCCGC > maxNumberOfCCGC) throw new DTIException(
				PaymentRules.class,
				DTIErrorCode.INVALID_PAYMENT_TYPE,
				"Number of credit and gift card payments provided " + numberOfCCGC + " exceeded " + tpiCode + " maximum of " + maxNumberOfCCGC);

		// Are any of the payment types being passed to DLR a voucher?
		if ((tpiCode.compareTo(DTITransactionTO.TPI_CODE_DLR) == 0) && (payListTO
				.size() > 0)) {
			for /* each */(PaymentTO aPaymentTO : /* in */payListTO) {
				if (aPaymentTO.getPayType() == PaymentTO.PaymentType.VOUCHER) {
					throw new DTIException(PaymentRules.class,
	...
	public static boolean determineTaxExemptStatus(String taxExemptCode) throws DTIException {
		...
		// Determine Tax Exempt Status
		if ((taxExemptCode != null) && (taxExemptCode.length() > 0)) isTaxExempt = true;
	...
	/**
	 * TO DO: JUNIT Ensures that transactions returning funds have a form of payment (either default or explicitly provided) to return funds to. Enforces the following rules: <BR>
	 * 1. Does the transaction have a form of payment to return funds to.
	 * 
	 * @param payListTO
	 *            An array list of Payment Transfer Objects.
	 ...
	 *             should any rule fail.
	 */
	public static void validateReturnFormOfPayment(
			ArrayList<PaymentTO> payListTO, EntityTO entityTO) throws DTIException {

		if (payListTO.size() > 0) return;

		if (entityTO.getDefPymtData() == null) throw new DTIException(
				PaymentRules.class, DTIErrorCode.INVALID_PAYMENT_TYPE,
				"Void transaction has no return form of payment, explicit or default.");
				...
				if (aPayment.getCreditCard() != null) {
					...
					if (aCreditCard.getCcVV() != null) {
						...
						if (cvv.length() < MIN_FIELD_LENGTH) {
							...
							// length
							// then a blank CCVV XML tag is technically legal.
							if (cvv.length() == 0) aCreditCard.setCcVV(null);
							else throw new DTIException(
									WDWReservationRules.class,
									DTIErrorCode.INVALID_MSG_CONTENT,
									"Credit card CVV length less than " + MIN_FIELD_LENGTH + " not allowed.  Length " + cvv
											.length() + " attempted.");
						}
					...
					if (aCreditCard.getCcStreet() != null) {
						...
						if (avsStreet.length() > MAX_WDW_AVS_STREET_LENGTH) {
							throw new DTIException(
									WDWReservationRules.class,
									DTIErrorCode.INVALID_MSG_CONTENT,
									"Credit card AVS Street length of " + avsStreet
											.length() + " is not allowed.  Maximum is " + MAX_WDW_AVS_STREET_LENGTH + ".");
						}
					...
					if (aCreditCard.getCcZipCode() != null) {
						...
						if (avsZip.length() > MAX_WDW_AVS_ZIP_LENGTH) {
							throw new DTIException(
									WDWReservationRules.class,
									DTIErrorCode.INVALID_MSG_CONTENT,
									"Credit card AVS ZIP length of " + avsZip
											.length() + " is not allowed.  Maximum is " + MAX_WDW_AVS_ZIP_LENGTH + ".");
						}
					...
					if (aCreditCard.getCcExpiration() != null) {
						...
						if (ccExpiration.length() != WDW_REQUIRED_EXPIRATION_LENGTH) {
							throw new DTIException(
									WDWReservationRules.class,
									DTIErrorCode.INVALID_MSG_CONTENT,
									"Credit card expiration length of " + ccExpiration
											.length() + " is not allowed.  Required length is " + WDW_REQUIRED_EXPIRATION_LENGTH + ".");
						}
					}
				}
			}
	...
	/**
	 * RULE: Validate that if the "installment" type of payment is present, the down-payment matches the required amount. (As of 2.16.1, JTL) RULE: If Installment Payment type is present, only one additional form of payment is allowed and
	 * it must be the same credit card as installment. (as of 2.16.1 JTL)
	 * 
	 * @param dtiTxn
	 * @param dtiResReq
	 * @param tpLookups
	 * @throws DTIException
	 */
	public static void validateResInstallDownpayment(DTITransactionTO dtiTxn,
			ArrayList<TPLookupTO> tpLookups) throws DTIException {
		...
		for (/* each */PaymentTO aPaymentTO : /* in */paymentList) {
			...
			if (aPaymentTO.getPayType() == PaymentTO.PaymentType.INSTALLMENT) {
				...
				if (aPaymentTO.getInstallment().getCreditCard()
						.getCcManualOrSwipe() == CreditCardTO.CreditCardType.CCMANUAL) {
					installmentCreditCard = aPaymentTO.getInstallment()
							.getCreditCard().getCcNbr();
				}
				else {
					installmentCreditCard = aPaymentTO.getInstallment()
							.getCreditCard().getCcTrack1();
				}
				...
				if (aPaymentTO.getCreditCard().getCcManualOrSwipe() == CreditCardTO.CreditCardType.CCMANUAL) {
					downpaymentCreditCard = aPaymentTO.getCreditCard()
							.getCcNbr();
				}
				else {
					downpaymentCreditCard = aPaymentTO.getCreditCard()
		...
		BigDecimal singleStandardDownpayment = null;
		for (/* each */TPLookupTO aTPLookup : /* in */tpLookups) {
			if ((aTPLookup.getLookupDesc().compareToIgnoreCase("Downpayment") == 0) && (aTPLookup
					.getLookupType() == TPLookupTO.TPLookupType.INSTALLMENT)) {

				try {
					singleStandardDownpayment = new BigDecimal(
							aTPLookup.getLookupValue());
				}
				catch (NumberFormatException nfe) {
					throw new DTIException(
							...
							"PaymentRules.validateInstallmentDownpayment attempted to read downpayment from DB, and it wasn't a number:" + aTPLookup
									.getLookupValue());
				}
			}
		}
		...
		// Determine how much down payment should be provided.
		BigDecimal stdPayTotal = singleStandardDownpayment
		...
		if (stdPayTotal.compareTo(installmentDownpayment) != 0) {
			throw new DTIException(
					DLRReservationRules.class,
					DTIErrorCode.INVALID_PAYMENT_AMOUNT,
					"The total downpayment expected of " + stdPayTotal + " does not equal what was on the reservation: " + installmentDownpayment);
		}
	...
	/**
	 * RULE: Validate that if the "installment" type of payment is present, the down-payment matches the required amount. (As of 2.16.1, JTL)
	 * 
	 * @param dtiTxn
	 * @param dtiResReq
	 * @param tpLookups
	 * @throws DTIException
	 */
	public static void validateRenewEntInstallDownpayment(
			DTITransactionTO dtiTxn, ArrayList<TPLookupTO> tpLookups) throws DTIException {
