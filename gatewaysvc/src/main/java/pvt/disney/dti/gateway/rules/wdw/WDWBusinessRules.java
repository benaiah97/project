...
>>>>>>> develop
	 ...
	 *             should any rule fail.
	 */
	static HashSet<Integer> validateTicketOrderShells(
			ArrayList<TicketTO> aTktListTO) throws DTIException {
	...
	private static String getShellStringFromTicket(TicketTO aTicketTO) {
		...
		ArrayList<TicketTO.TicketIdType> ticketTypes = aTicketTO
				.getTicketTypes();
	 ...
	 */
	static OTHeaderTO transformOTHeader(DTITransactionTO dtiTxn,
			String requestType, String requestSubType) throws DTIException {
		...
		AttributeTO anAttributeTO = aMap
				.get(AttributeTO.CmdAttrCodeType.OP_AREA);
		...
		if (anAttributeTO == null) {
			throw new DTIException(WDWBusinessRules.class,
					DTIErrorCode.DTI_DATA_ERROR,
					"User ID not set up in DTI database.");
		}
	 ...
	 */
	static void createOTPaymentList(ArrayList<OTPaymentTO> otPaymentList,
		...
		if ((dtiPayList == null) || (dtiPayList.size() == 0)) {
			...
			if ((entityTO.getDefPymtIdEnum() == DefaultPaymentType.VOUCHER)
				...
				dtiPaymentTO.setPayItem(new BigInteger("1")); // There will only
																// be one
				// default payment.
	 ...
	 */
	private static OTPaymentTO transformOTPaymentRequest(
			PaymentTO dtiPaymentTO, EntityTO entityTO) {
	 ...
	 * @return a populated OT Payment transfer object.
	 */
	private static OTPaymentTO transformOTCreditCardRequest(
			PaymentTO dtiPayment, EntityTO entityTO) {
		...
		CreditCardTO.CreditCardType entryType = dtiCreditCard
				.getCcManualOrSwipe();
		...
		if (entryType == CreditCardTO.CreditCardType.CCSWIPE) {
			...
			if (dtiCreditCard.getExtnlDevSerial() != null) { // As of 2.12
				otCreditCard.setExternalDeviceSerialId(dtiCreditCard
						.getExtnlDevSerial());
			}
	 ...
	 * @return a populated OTPaymentTO object populated for voucher.
	 */
	private static OTPaymentTO transformOTInstallmentRequest(
			PaymentTO dtiPayment) {
		...
		CreditCardType cardType = dtiInstallTO.getCreditCard()
				.getCcManualOrSwipe();
	...
	static void setOTInTransactionAttributes(DTITransactionTO dtiTxn,
		...
		if ((cmdActor == null) || (cmdActor.length() == 0)
			...
			// TSMAC.TSLOC
			String tsMac = dtiRequest.getPayloadHeader().getTktSeller()
					.getTsMac();
			String tsLocation = dtiRequest.getPayloadHeader().getTktSeller()
					.getTsLocation();
			inTxnAttr = new OTInTransactionAttributeTO();
			inTxnAttr.setAttributeCmd(INSERTCMD);
	 ...
	 */
	static OTTicketTO setOTTicketTO(TicketTO dtiTicket,
			TicketIdType dtiTicketType) {
...
=======
		if (otCmdTO == null) throw new DTIException(
				...
				DTIErrorCode.UNDEFINED_FAILURE,
				"Internal Error:  Omni XML translated into a response with null Command object.");
		if (otCmdTO.getHeaderTO() == null) throw new DTIException(
				TransformRules.class, DTIErrorCode.UNDEFINED_FAILURE,
				"Internal Error:  Omni XML translated into a response with null Header object.");
		if (otCmdTO.hasBodyObject() == false) throw new DTIException(
				TransformRules.class, DTIErrorCode.UNDEFINED_FAILURE,
				"Internal Error:  Omni XML translated into a response with null body object.");
...
>>>>>>> develop
			...
			case MULTIENTITLEMENTACCOUNT:
				...
				// 2.16.1 BIEST001
				// The MultiEntitlementAccount signature is quite diverse in the number of functions
				// that it controls in the RFID space. It provides the ability to Create Entitlements (legacy and electronic),
				// Transfer Entitlements from one account to another, Order Entitlements so that certain tickets have usage applied first,
				// Create Electronic Accounts w/ or w/out media and entitlements on them, Query Account, Merge the contents of one account
				// to a different account, Change the Status of an account, Associate Media to an Account, Deassociate Media from an Account,
				// and Change the Status of Media.

				// The response comes back identical for all of these things...but we need to know what function was originally called

				switch (dtiTransType) {
				case ASSOCIATEMEDIATOACCOUNT:
					WDWAssociateMediaToAccountRules.transformResponseBody(
							dtiTxn, otCmdTO, dtiRespTO);
					break;
				case TICKERATEENTITLEMENT:
					WDWTickerateEntitlementRules.transformResponseBody(dtiTxn,
							otCmdTO, dtiRespTO);
					break;
			...
			case MANAGERESERVATION: // As of 2.16.1 BIEST001
				...
				case QUERYRESERVATION:
					WDWQueryReservationRules.transformResponseBody(dtiTxn,
							otCmdTO, dtiRespTO);
					break;
        ...
        WDWQueryTicketRules.transformResponseBody(dtiTxn, otCmdTO, dtiRespTO);
			...
			case UPDATETRANSACTION:
				WDWUpdateTransactionRules.transformResponseBody(dtiTxn,
						otCmdTO, dtiRespTO);
				break;
...
>>>>>>> develop
	 ...
	 */
	static void setDTIPaymentList(ArrayList<PaymentTO> dtiPmtList,
			ArrayList<OTPaymentTO> otPmtList) {
	 ...
	 * @param otDemoInfo
	 */
	public static void transformTicketDemoData(
			ArrayList<DemographicsTO> tktDemoList, OTDemographicInfo otDemoInfo) {
		...
		for /* each */(DemographicsTO aDtiTicket : /* in */tktDemoList) {
			...
			String dobString = sdf
					.format(aDtiTicket.getDateOfBirth().getTime());
			otDemoData.addOTField(new OTFieldTO(
					OTFieldTO.TKT_DEMO_DATE_OF_BIRTH, dobString));
...
>>>>>>> develop
			...
			otDemoData.addOTField(new OTFieldTO(OTFieldTO.TKT_DEMO_ADDRESS_ONE,
					DTIFormatter.websafe(aDtiTicket.getAddr1().toUpperCase())));

			// Address 2 (optional)
			if (aDtiTicket.getAddr2() != null) {
				otDemoData.addOTField(new OTFieldTO(
						OTFieldTO.TKT_DEMO_ADDRESS_TWO, DTIFormatter
								.websafe(aDtiTicket.getAddr2().toUpperCase())));
			}

			// City
			otDemoData.addOTField(new OTFieldTO(OTFieldTO.TKT_DEMO_CITY,
					DTIFormatter.websafe(aDtiTicket.getCity().toUpperCase())));

			// State (optional)
			if (aDtiTicket.getState() != null) {
				otDemoData.addOTField(new OTFieldTO(OTFieldTO.TKT_DEMO_STATE,
						DTIFormatter.websafe(aDtiTicket.getState()
								.toUpperCase())));
			}

			// ZIP
			otDemoData.addOTField(new OTFieldTO(OTFieldTO.TKT_DEMO_ZIP,
					DTIFormatter.websafe(aDtiTicket.getZip())));

			// Country
			otDemoData
					.addOTField(new OTFieldTO(OTFieldTO.TKT_DEMO_COUNTRY,
							DTIFormatter.websafe(aDtiTicket.getCountry()
									.toUpperCase())));

			// Telephone
			otDemoData.addOTField(new OTFieldTO(OTFieldTO.TKT_DEMO_TELEPHONE,
					DTIFormatter.websafe(aDtiTicket.getTelephone()
							.toUpperCase())));

			// Email (optional)
			if (aDtiTicket.getEmail() != null) {
				otDemoData.addOTField(new OTFieldTO(OTFieldTO.TKT_DEMO_EMAIL,
						DTIFormatter.websafe(aDtiTicket.getEmail()
								.toUpperCase())));
			}
	 ...
	 * @param otDemoInfo
	 */
	public static void transformInstallmentDemoData(
			InstallmentDemographicsTO installDemoTO,
			ArrayList<OTFieldTO> otDemoList) {

		// FirstName
		String firstNameString = installDemoTO.getFirstName();
		OTFieldTO aField = new OTFieldTO(OTFieldTO.INST_DEMO_FIRSTNAME,
				firstNameString);
		...
		if (installDemoTO.getMiddleName() != null) {
			...
			aField = new OTFieldTO(OTFieldTO.INST_DEMO_MIDLNAME,
					middleNameString);
			otDemoList.add(aField);
		}
		...
		if (installDemoTO.getDateOfBirth() != null) {
			SimpleDateFormat sdf = new SimpleDateFormat("MM/dd/yy");
			String dobString = sdf.format(installDemoTO.getDateOfBirth()
					.getTime());
