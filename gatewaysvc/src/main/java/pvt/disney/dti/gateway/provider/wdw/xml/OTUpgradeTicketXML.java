...
<<<<<<< HEAD
  /**
   * Adds the transaction body element to the command stanza XML based upon the
   * transfer object provided.
   * 
   * @param otUpgrdTktTO
   ...
   */
  static void addTxnBodyElement(OTUpgradeTicketTO otUpgrdTktTO, Element commandStanza) {
    ...
    // SiteNumber
    upgrdTktStanza.addElement("SiteNumber").addText(otUpgrdTktTO.getSiteNumber().toString());

    // UpgradeTicketInfo
    ArrayList<OTUpgradeTicketInfoTO> tktList = otUpgrdTktTO.getUpgradeTicketInfoList();
    for /* each */(OTUpgradeTicketInfoTO otTicketInfo : /* in */tktList) {

      Element upgrdTicketInfoStanza = upgrdTktStanza.addElement("UpgradeTicketInfo");

      // Item
      upgrdTicketInfoStanza.addElement("Item").addText(otTicketInfo.getItem().toString());

      // TicketSearchMode
      Element tktSrchModeStanza = upgrdTicketInfoStanza.addElement("TicketSearchMode");
      ...
      // TicketType
      upgrdTicketInfoStanza.addElement("TicketType").addText(otTicketInfo.getTicketType().toString());

      // Price (ignored)

      // Validity
      if ((otTicketInfo.getValidityStartDate() != null) || (otTicketInfo.getValidityEndDate() != null)) {
        Element validityElement = upgrdTicketInfoStanza.addElement("Validity");

        if (otTicketInfo.getValidityStartDate() != null) {
          GregorianCalendar startDateCal = otTicketInfo.getValidityStartDate();
          String startDateString = CustomDataTypeConverter.printCalToDTIDateFormat(startDateCal);
          validityElement.addElement("StartDate").addText(startDateString);
        }

        if (otTicketInfo.getValidityEndDate() != null) {
          GregorianCalendar endDateCal = otTicketInfo.getValidityEndDate();
          String endDateString = CustomDataTypeConverter.printCalToDTIDateFormat(endDateCal);
          validityElement.addElement("EndDate").addText(endDateString);
        }
      ...
      if (otTicketInfo.getTicketNote() != null)
        upgrdTicketInfoStanza.addElement("TicketNote").addText(otTicketInfo.getTicketNote());
      ...
      if ((otDemoInfo != null) && (otDemoInfo.getDemoDataList().size() > 0)) {
        ...
        ArrayList<OTDemographicData> otDemoDataList = otDemoInfo.getDemoDataList();

        for /* each */(OTDemographicData oTDemoData : /* in */otDemoDataList) {

          Element demoDataStanza = upgrdTicketInfoStanza.addElement("DemographicData");
    ...
    // PaymentInfo
    if (otUpgrdTktTO.getPaymentInfoList() != null)
      OTCommandXML.addPaymentInfoStanza(upgrdTktStanza, otUpgrdTktTO.getPaymentInfoList());

    // SellerId
    if (otUpgrdTktTO.getSellerId() != null)
      upgrdTktStanza.addElement("SellerId").addText(otUpgrdTktTO.getSellerId().toString());

    // AssociationInfo
    if (otUpgrdTktTO.getAssociationId() != null) {

      Element assnInfoStanza = upgrdTktStanza.addElement("AssociationInfo");

      assnInfoStanza.addElement("AssociationId").addText(otUpgrdTktTO.getAssociationId().toString());

      Element memberInfo = assnInfoStanza.addElement("MemberInfo");

      if (otUpgrdTktTO.getMemberId() != null)
        memberInfo.addElement("MemberId").addText(otUpgrdTktTO.getMemberId().toString());
      else
        memberInfo.addElement("MemberField").addText(otUpgrdTktTO.getMemberField());
    }

    // IATA
    if (otUpgrdTktTO.getIATA() != null)
      upgrdTktStanza.addElement("IATA").addText(otUpgrdTktTO.getIATA());

    // TransactionNote
    if (otUpgrdTktTO.getTransactionNote() != null)
      upgrdTktStanza.addElement("TransactionNote").addText(otUpgrdTktTO.getTransactionNote());

    // InTransaction Attribute
    ArrayList<OTInTransactionAttributeTO> inTxnAttrList = otUpgrdTktTO.getInTxnAttrList();

    if (inTxnAttrList != null)
      OTCommandXML.addInTransactionAttributeStanzas(upgrdTktStanza, inTxnAttrList);

    // DenyUpgradeOnUpgrade
    if (otUpgrdTktTO.getDenyUpgradeOnUpgrade() != null)
      upgrdTktStanza.addElement("DenyUpgradeOnUpgrade").addText(otUpgrdTktTO.getDenyUpgradeOnUpgrade().toString());

    // GenerateEvent
    if (otUpgrdTktTO.getGenerateEvent() != null) {
      upgrdTktStanza.addElement("GenerateEvent").addText(otUpgrdTktTO.getGenerateEvent().toString());
    }
  ...
  /**
   * When provided with the parsed XML, returns the transfer object for the
   * upgrade ticket.
   * 
   * @param upgrdTktNode
  ...
  static OTUpgradeTicketTO getTO(Node upgrdTktNode) throws DTIException {
    ...
    // UpgradeTicketInfo
    List<Node> upgradeTicketInfoNodeList = upgrdTktNode.selectNodes("UpgradeTicketInfo");
    if ((upgradeTicketInfoNodeList != null) && (upgradeTicketInfoNodeList.size() > 0))
      setOTUpgradeTicketInfoTOList(otUpgrdTktTO.getUpgradeTicketInfoList(), upgradeTicketInfoNodeList);

    // PaymentInfo
    List<Node> paymentInfoNodeList = upgrdTktNode.selectNodes("PaymentInfo");
    if (paymentInfoNodeList != null)
      OTCommandXML.setOTPaymentTOList(otUpgrdTktTO.getPaymentInfoList(), paymentInfoNodeList);

    // TransactionDSSN
    Node tranDssnNode = upgrdTktNode.selectSingleNode("TransactionDSSN");
    if (tranDssnNode != null)
      otUpgrdTktTO.setTransactionDSSN(OTCommandXML.setOTTransactionDSSNTO(tranDssnNode));

    // TransactionCOD
    Node tranCODNode = upgrdTktNode.selectSingleNode("TransactionCOD");
    if (tranCODNode != null)
      otUpgrdTktTO.setTransactionCOD(tranCODNode.getText());
  ...
  /**
   * Sets the transfer object for upgrade ticket info based upon the parsed XML
   * provided.
   * 
   * @param otTktList
   ...
   *           for any missing required fields or other parsing errors.
   */
  private static void setOTUpgradeTicketInfoTOList(ArrayList<OTUpgradeTicketInfoTO> otTktList,
    ...
    for /* each */(Node aNode : /* in */ticketList) {
      ...
      if (itemNode == null)
        throw new DTIException(OTUpgradeTicketXML.class, DTIErrorCode.TP_INTERFACE_FAILURE,
