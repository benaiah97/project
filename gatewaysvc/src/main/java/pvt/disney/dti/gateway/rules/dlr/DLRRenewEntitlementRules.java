...
/**
<<<<<<< HEAD
 * The DLR Renew Entitlement Rules contains the methods to transform the
 * request, transform the response, and evaluate the specific business rules for
 * this transaction.
=======
 * The DLR Renew Entitlement Rules contains the methods to transform the request, transform the response, and evaluate the specific business rules for this transaction.
>>>>>>> develop
 * 
 * @author lewit019
 * @since 2.16.1
 */
public class DLRRenewEntitlementRules implements TransformConstants {
...
<<<<<<< HEAD
  ...
  static String transformRequest(DTITransactionTO dtiTxn) throws DTIException {
    ...
    RenewEntitlementRequestTO renewReq = (RenewEntitlementRequestTO) dtiTxn.getRequest().getCommandBody();
    ...
    // clause
    if (renewReq.isInstallmentRenewRequest()) {
      ...
      if (renewReq.getInstallmentDownpayment() != null) {
        String downPaymentAmount = renewReq.getInstallmentDownpayment().toString();
      ...
      // PaymentPlan
      String eligMember = renewReq.getEligibilityMember();

      if (eligMember.equalsIgnoreCase(SOCA_RES)) {
        String paymentPlan = instLookupMap.get(GW_ORDERS_SOCARENEWPLAN);
        if (paymentPlan == null) {
          throw new DTIException(TransformRules.class, DTIErrorCode.UNDEFINED_CRITICAL_ERROR,
              "Internal Error:  DLR Installmnt paymentPlan not in TP_LOOKUP Table for " + GW_ORDERS_SOCARENEWPLAN + ".");
        }
        contract.setPaymentPlanID(paymentPlan);
      } else if (eligMember.equalsIgnoreCase(CA_RES)) {
        String paymentPlan = instLookupMap.get(GW_ORDERS_CARENEWPLAN);
        if (paymentPlan == null) {
          throw new DTIException(TransformRules.class, DTIErrorCode.UNDEFINED_CRITICAL_ERROR,
              "Internal Error:  DLR Installmnt paymentPlan not in TP_LOOKUP Table for " + GW_ORDERS_CARENEWPLAN + ".");
        }
        contract.setPaymentPlanID(paymentPlan);
      }
    ...
    ArrayList<TicketTO> dtiTktList = renewReq.getTktList();
    ...
    BigDecimal orderTotal = setProductOrderLines(renewReq, orderTO, dtiProductMap, dtiTktList);
    ...
    boolean missingCvv = setPaymentOrderLines(renewReq, orderTO, orderTotal, payCardMap);
  ...
  private static GWOrderTO createOrderTO(DTITransactionTO dtiTxn) throws DTIException {
    ...
    HashMap<TPLookupTO.TPLookupType, TPLookupTO> gwTPLookupMap = dtiTxn.getTpLookupTOMap();
    ...
    try {
      ...
      orderTO.setShipDeliveryDetails("Automatic Activation"); // TODO details
                                                              // for renewals.

      orderTO.setShipDeliveryMethod("17"); // TODO delivery method for renewals.
  ...
  /**
   * Transforms the eGalaxy XML response string into response objects within the
   * DTI transfer object.
   * 
   * @param dtiTxn
  ...
  static DTITransactionTO transformResponse(DTITransactionTO dtiTxn, String xmlResponse) throws DTIException {
    ...
    RenewEntitlementResponseTO dtiRenewEntRespTO = new RenewEntitlementResponseTO();
    ...
    dtiRenewEntRespTO.addPayment(paymentTO);

    // set contract (if there) as of 2.16.1, JTL
    if (gwOrdResp.getPaymentContractID() != null) {
      dtiRenewEntRespTO.setContractId(gwOrdResp.getPaymentContractID());
    }
    ...
    dtiRenewEntRespTO.setClientData(clientDataTO);
    ...
    if (gwTicketArray.size() > 0) {
      ...
      for (int i = 0; gwTicketArray.size() > i; i++) {
        ...
        dtiRenewEntRespTO.addTicket(dtiTicket);
    ...
    dtiRespTO.setCommandBody(dtiRenewEntRespTO);
   ...
   *           the DTI exception
   */
  public static void applyDLRRenewEntitlementRules(DTITransactionTO dtiTxn) throws DTIException {
        ...
        for /* each */(DemographicsTO aDemo : /* in */demoListTO) {
          if (aDemo.getCountry() != null) {
            if (aDemo.getCountry().length() != 2) {
              throw new DTIException(DLRReservationRules.class, DTIErrorCode.INVALID_MSG_CONTENT,
                  "DLR Demographics country code must be length 2 - length found was " + aDemo.getCountry().length());
            }
          }
   ...
   */
  private static BigDecimal setProductOrderLines(RenewEntitlementRequestTO renEntReq, GWOrderTO orderTO,
    ...
    while (iter.hasNext()) {
      ...
      // PaymentPlanID
      if (renEntReq.isInstallmentRenewRequest()) {
      ...
      // Eligibility group
      if (renEntReq.getEligibilityGroup() != null) {

        if ((renEntReq.getEligibilityGroup()
            .equalsIgnoreCase(TransformConstants.GW_ORDERS_DLR_MTGS_CONV_ELIGIBILITY_GROUP))
            || (renEntReq.getEligibilityGroup()
                .equalsIgnoreCase(TransformConstants.GW_ORDERS_DLR_WEDDINGS_ELIGIBILITY_GROUP))
            || (renEntReq.getEligibilityGroup().equalsIgnoreCase(TransformConstants.GW_ORDERS_DLR_BOLT_GROUP))) {

          if (dtiTicket.getTktValidityValidStart() != null) {
            gwLineItemTO.setTicketDate(UtilityXML.getEGalaxyDateFromGCal(dtiTicket.getTktValidityValidStart()));
          }
        }
      }
   ...
   */
  private static boolean setPaymentOrderLines(RenewEntitlementRequestTO renewEntReq, GWOrderTO orderTO,
      BigDecimal orderTotal, HashMap<String, String> payCardMap) {
    String paymentCode = null;
    boolean missingCvv = false;

    Iterator<PaymentTO> paymentIter = renewEntReq.getPaymentList().iterator();
...
=======
	...
	static String transformRequest(DTITransactionTO dtiTxn) throws DTIException {
		...
		RenewEntitlementRequestTO renewReq = (RenewEntitlementRequestTO) dtiTxn
		...
		if (renewReq.isInstallmentRenewRequest()) {
			...
			contract.setPaymentContractStatusID(GW_ORDERS_CONTRACT_STATUS_ID);
			if (renewReq.getInstallmentDownpayment() != null) {
				String downPaymentAmount = renewReq.getInstallmentDownpayment()
						.toString();
				contract.setDownPaymentAmount(downPaymentAmount);
			}
			contract.setContactMethod(GW_ORDERS_CONTRACT_CONTACT_METHOD);
		...
		// Set the order lines, returning the order total (required by the header)
		BigDecimal orderTotal = setProductOrderLines(renewReq, orderTO,
				dtiProductMap, dtiTktList);
		...
		// use this to get order line item that are payments
		HashMap<String, String> payCardMap = dtiTxn.getPaymentCardMap();
		boolean missingCvv = setPaymentOrderLines(renewReq, orderTO,
				orderTotal, payCardMap);
	...
	private static GWOrderTO createOrderTO(DTITransactionTO dtiTxn) throws DTIException {
		...
		if (resReq.isInstallmentRenewRequest()) {
			...
			if (paymentPlan == null) {
				throw new DTIException(
						...
						DTIErrorCode.UNDEFINED_FAILURE,
						"GWTPLookup for " + GW_ORDERS_SOCARENEWPLAN + " or " + GW_ORDERS_CARENEWPLAN + " is missing in the database.");
			}
			payContract.setPaymentPlanID(paymentPlan);
		...
		try {
			...
			orderTO.setShipDeliveryMethod(GW_SHIP_DELIVERY); // delivery method for renewals.
	...
	public static void applyDLRRenewEntitlementRules(DTITransactionTO dtiTxn) throws DTIException {
		...
		if (tpLookups.size() == 0) {
			throw new DTIException(
					...
					"TPCommandLookup query on renew entitlement did not return with responses as expected (set-up incomplete): " + tpLookups
							.size());
		}
		dtiTxn.setTpLookupTOList(tpLookups);
	 ...
	 * @return the order total (needed in the GW header)
	 */
	private static BigDecimal setProductOrderLines(
			RenewEntitlementRequestTO renEntReq, GWOrderTO orderTO,
		...
		while (iter.hasNext()) {
			...
			// Set the detail type based on the type of product
			if ((dtiProduct.getDaySubclass().equalsIgnoreCase(GW_RENEW)) || (dtiProduct
					.getDaySubclass().equalsIgnoreCase(GW_PASS))) {
				gwLineItemTO
			...
			// PLU (mapped from Ticket Name, not ticket code because of length)
			DBProductTO dtiDbProduct = dtiProductMap.get(dtiTicket
					.getProdCode());
			gwLineItemTO.setPlu(dtiDbProduct.getMappedProviderTktName());
			...
			if (renEntReq.getEligibilityGroup() != null) {
				...
				if ((renEntReq.getEligibilityGroup()
						.equalsIgnoreCase(TransformConstants.GW_ORDERS_DLR_MTGS_CONV_ELIGIBILITY_GROUP)) || (renEntReq
						.getEligibilityGroup()
						.equalsIgnoreCase(TransformConstants.GW_ORDERS_DLR_WEDDINGS_ELIGIBILITY_GROUP)) || (renEntReq
						.getEligibilityGroup()
			...
			if (dtiTicket.getTicketDemoList().size() != 0) {
				...
				// Visual ID (as of 2.16.1, JTL)
				if (dtiTicket.getExistingTktID().getExternal() != null) {
					gwDemo.setVisualID(dtiTicket.getExistingTktID()
							.getExternal());
				}
	 ...
	 * @return true or false if the CVV was found or not.
	 */
	private static boolean setPaymentOrderLines(
			RenewEntitlementRequestTO renewEntReq, GWOrderTO orderTO,
			BigDecimal orderTotal, HashMap<String, String> payCardMap) {
		String paymentCode = null;
		boolean missingCvv = false;

		Iterator<PaymentTO> paymentIter = renewEntReq.getPaymentList()
				.iterator();
		...
		while (paymentIter.hasNext()) {
			...
			if (paymentTO.getPayType() == PaymentType.CREDITCARD) {
				...
				// set pay amount
				gwOrderLinePayTO.setAmount(String.valueOf(paymentTO
						.getPayAmount()));
			}
