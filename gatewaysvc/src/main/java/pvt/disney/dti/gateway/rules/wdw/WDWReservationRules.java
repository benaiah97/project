...
<<<<<<< HEAD
  ...
  static String transformRequest(DTITransactionTO dtiTxn) throws DTIException {
    ...
    if (numberOfTickets > atsMaxEncodeAllCnt) {
      ...
      logger.sendEvent("Number of Tickets (" + numberOfTickets + ") is less than or equal to " + 
          "ATSMaxEncodeAllCnt (" + atsMaxEncodeAllCnt + ")", EventType.DEBUG, THISOBJECT);
    }
  ...
  private static void createOtDemographics(ArrayList<OTFieldTO> otFieldList, DemographicsTO dtiBillInfo,
    ...
    if (dtiBillInfo != null) {
      ...
      if (dtiBillInfo.getName() != null)
        otFieldList.add(new OTFieldTO(OTFieldTO.RES_BILL_NAME, DTIFormatter.websafe(dtiBillInfo.getName())));

      // Bill Last Name
      if (dtiBillInfo.getLastName() != null)
        otFieldList.add(new OTFieldTO(OTFieldTO.RES_BILL_LASTNAME, DTIFormatter.websafe(dtiBillInfo
            .getLastName())));

      // Bill First Name
      if (dtiBillInfo.getFirstName() != null)
        otFieldList.add(new OTFieldTO(OTFieldTO.RES_BILL_FIRSTNAME, DTIFormatter.websafe(dtiBillInfo
            .getFirstName())));

      // Bill Address 1
      if (dtiBillInfo.getAddr1() != null)
        otFieldList
            .add(new OTFieldTO(OTFieldTO.RES_BILL_ADDR1, DTIFormatter.websafe(dtiBillInfo.getAddr1())));

      // Bill Address 2
      if (dtiBillInfo.getAddr2() != null)
        otFieldList
            .add(new OTFieldTO(OTFieldTO.RES_BILL_ADDR2, DTIFormatter.websafe(dtiBillInfo.getAddr2())));

      // Bill City
      if (dtiBillInfo.getCity() != null)
        otFieldList.add(new OTFieldTO(OTFieldTO.RES_BILL_CITY, DTIFormatter.websafe(dtiBillInfo.getCity())));

      // Bill State
      if (dtiBillInfo.getState() != null)
        otFieldList
            .add(new OTFieldTO(OTFieldTO.RES_BILL_STATE, DTIFormatter.websafe(dtiBillInfo.getState())));

      // Bill ZIP
      if (dtiBillInfo.getZip() != null)
        otFieldList.add(new OTFieldTO(OTFieldTO.RES_BILL_ZIP, DTIFormatter.websafe(dtiBillInfo.getZip())));

      // Bill Country
      if (dtiBillInfo.getCountry() != null)
        otFieldList.add(new OTFieldTO(OTFieldTO.RES_BILL_COUNTRY, DTIFormatter.websafe(dtiBillInfo
            .getCountry())));

      // Bill Telephone
      if (dtiBillInfo.getTelephone() != null)
        otFieldList.add(new OTFieldTO(OTFieldTO.RES_BILL_TELEPHONE, DTIFormatter.websafe(dtiBillInfo
            .getTelephone())));

      // Bill E-mail
      if (dtiBillInfo.getEmail() != null)
        otFieldList
            .add(new OTFieldTO(OTFieldTO.RES_BILL_EMAIL, DTIFormatter.websafe(dtiBillInfo.getEmail())));

      // Bill SellerResNbr
      if (dtiBillInfo.getSellerResNbr() != null)
        otFieldList.add(new OTFieldTO(OTFieldTO.RES_BILL_SLR_RES_NBR, DTIFormatter.websafe(dtiBillInfo
            .getSellerResNbr())));
    }
    ...
    if (dtiShipInfo != null) {
      ...
      if (dtiShipInfo.getName() != null)
        otFieldList.add(new OTFieldTO(OTFieldTO.RES_SHIP_NAME, DTIFormatter.websafe(dtiShipInfo.getName())));

      // Ship Last Name
      if (dtiShipInfo.getLastName() != null)
        otFieldList.add(new OTFieldTO(OTFieldTO.RES_SHIP_LASTNAME, DTIFormatter.websafe(dtiShipInfo
            .getLastName())));

      // Ship First Name
      if (dtiShipInfo.getFirstName() != null)
        otFieldList.add(new OTFieldTO(OTFieldTO.RES_SHIP_FIRSTNAME, DTIFormatter.websafe(dtiShipInfo
            .getFirstName())));

      // Ship Address 1
      if (dtiShipInfo.getAddr1() != null)
        otFieldList
            .add(new OTFieldTO(OTFieldTO.RES_SHIP_ADDR1, DTIFormatter.websafe(dtiShipInfo.getAddr1())));

      // Ship Address 2
      if (dtiShipInfo.getAddr2() != null)
        otFieldList
            .add(new OTFieldTO(OTFieldTO.RES_SHIP_ADDR2, DTIFormatter.websafe(dtiShipInfo.getAddr2())));

      // Ship City
      if (dtiShipInfo.getCity() != null)
        otFieldList.add(new OTFieldTO(OTFieldTO.RES_SHIP_CITY, DTIFormatter.websafe(dtiShipInfo.getCity())));

      // Ship State
      if (dtiShipInfo.getState() != null)
        otFieldList
            .add(new OTFieldTO(OTFieldTO.RES_SHIP_STATE, DTIFormatter.websafe(dtiShipInfo.getState())));

      // Ship ZIP
      if (dtiShipInfo.getZip() != null)
        otFieldList.add(new OTFieldTO(OTFieldTO.RES_SHIP_ZIP, DTIFormatter.websafe(dtiShipInfo.getZip())));

      // Ship Country
      if (dtiShipInfo.getCountry() != null)
        otFieldList.add(new OTFieldTO(OTFieldTO.RES_SHIP_COUNTRY, DTIFormatter.websafe(dtiShipInfo
            .getCountry())));

      // Ship Telephone
      if (dtiShipInfo.getTelephone() != null)
        otFieldList.add(new OTFieldTO(+OTFieldTO.RES_SHIP_TELEPHONE, DTIFormatter.websafe(dtiShipInfo
            .getTelephone())));
      ...
      if (agencyTO != null) {
        if (agencyTO.getAgent() != null) {
          otFieldList
              .add(new OTFieldTO(OTFieldTO.RES_SHIP_AGENT, DTIFormatter.websafe(agencyTO.getAgent())));
        }
      }
    }
   ...
   * maximum sizes on CVV & AVS data, if present.
    ...
    for (SpecifiedAccountTO dtiAccount : dtiResReq.getSpecifiedAccounts()) {
      ...
      if (dtiAccount.getAccountDemo() != null) {
        ...
        if (dtiAcctDemo.getTitle() != null) {
          otFieldList.add(new OTFieldTO(OTFieldTO.ACCT_DEMO_TITLE, DTIFormatter.websafeToUpper(dtiAcctDemo.getTitle())));
        }
        
        // FirstName
        if (dtiAcctDemo.getFirstName() != null) {
          otFieldList.add(new OTFieldTO(OTFieldTO.ACCT_DEMO_FIRSTNAME, DTIFormatter.websafeToUpper(dtiAcctDemo.getFirstName())));          
        }
        
        // MiddleName
        if (dtiAcctDemo.getMiddleName() != null) {
          otFieldList.add(new OTFieldTO(OTFieldTO.ACCT_DEMO_MIDDLENAME, DTIFormatter.websafeToUpper(dtiAcctDemo.getMiddleName())));
        }
        
        // LastName
        if (dtiAcctDemo.getLastName() != null) {
          otFieldList.add(new OTFieldTO(OTFieldTO.ACCT_DEMO_LASTNAME, DTIFormatter.websafeToUpper(dtiAcctDemo.getLastName())));
        }
        
        // Suffix
        if (dtiAcctDemo.getSuffix() != null) {
          otFieldList.add(new OTFieldTO(OTFieldTO.ACCT_DEMO_SUFFIX, DTIFormatter.websafeToUpper(dtiAcctDemo.getSuffix())));
        }
        ...
        if (dtiAcctDemo.getDateOfBirth() != null) {
          ...
          otFieldList.add(new OTFieldTO(OTFieldTO.ACCT_DEMO_DOB, dobString));
        }
        
        // Addr1
        if (dtiAcctDemo.getAddr1() != null) {
          otFieldList.add(new OTFieldTO(OTFieldTO.ACCT_DEMO_ADDRESS1, DTIFormatter.websafeToUpper(dtiAcctDemo.getAddr1())));
        }
        
        // Addr2
        if (dtiAcctDemo.getAddr2() != null) {
          otFieldList.add(new OTFieldTO(OTFieldTO.ACCT_DEMO_ADDRESS2, DTIFormatter.websafeToUpper(dtiAcctDemo.getAddr2())));
        }
        
        // City
        if (dtiAcctDemo.getCity() != null) {
          otFieldList.add(new OTFieldTO(OTFieldTO.ACCT_DEMO_CITY, DTIFormatter.websafeToUpper(dtiAcctDemo.getCity())));
        }
        
        // State
        if (dtiAcctDemo.getState() != null) {
          otFieldList.add(new OTFieldTO(OTFieldTO.ACCT_DEMO_STATE, DTIFormatter.websafeToUpper(dtiAcctDemo.getState())));
        }
        
        // ZIP
        if (dtiAcctDemo.getZip() != null) {
          otFieldList.add(new OTFieldTO(OTFieldTO.ACCT_DEMO_ZIP, DTIFormatter.websafeToUpper(dtiAcctDemo.getZip())));
        }
        
        // Country
        if (dtiAcctDemo.getCountry() != null) {
          otFieldList.add(new OTFieldTO(OTFieldTO.ACCT_DEMO_COUNTRY, DTIFormatter.websafeToUpper(dtiAcctDemo.getCountry())));
        }
        
        // Email
        if (dtiAcctDemo.getEmail() != null) {
          otFieldList.add(new OTFieldTO(OTFieldTO.ACCT_DEMO_EMAIL, DTIFormatter.websafeToUpper(dtiAcctDemo.getEmail())));
        }
        
        // OptInSolicit
        if (dtiAcctDemo.getOptInSolicit() != null) {
          if (dtiAcctDemo.getOptInSolicit().booleanValue() == true) {
            otFieldList.add(new OTFieldTO(OTFieldTO.ACCT_DEMO_OKTOMAIL, WDWBusinessRules.YES));
          } else {
            otFieldList.add(new OTFieldTO(OTFieldTO.ACCT_DEMO_OKTOMAIL, WDWBusinessRules.NO));
          }
        }
