...
/**
 * This class has the responsibility to transform between DTITransactionTO objects and the provider's specific request string format.
 * 
 * @author lewit019
 */
public abstract class TransformRules {
	...
	/**
	 * This method changes from the DTITransaction to any provider's request string format. It determines the specific provider to use based on the tpiCode stored in the dtiTxn object.
	 * 
	 * @param dtiTxn
	...
	/**
	 * Parses the xmlResponse string from any provider into the DTITransactionTO object.
	 * 
	 * @param dtiTxn
	 ...
	 *             for any error. Contains enough detail to formulate an error response to the seller.
	 */
	public static DTITransactionTO changeToDtiFormat(DTITransactionTO dtiTxn,
			String xmlResponse) throws DTIException {

		String tpiCode = dtiTxn.getTpiCode();
		if (tpiCode.equals(DTITransactionTO.TPI_CODE_WDW)) {

			return WDWBusinessRules.changeWDWProviderFormatToDti(dtiTxn,
					xmlResponse);

		}
		else if (tpiCode.equals(DTITransactionTO.TPI_CODE_DLR)) {

			return DLRBusinessRules.changeDLRProviderFormatToDti(dtiTxn,
					xmlResponse);
	...
	public static String setFulfillmentNoteDetails(DTITransactionTO dtiTxn) {
		...
		// fields are provided.
		if ((dtiClientData.getGroupValidityValidStart() != null) || (dtiClientData
				.getGroupValidityValidEnd() != null) || sb.length() > 0 || dtiClientData
				.getClientSalesContact() != null) {
