 ...
 * internal objects used by the DTI Application. This class manages the response
 * version of the transmission clause.
 * 
 * This class is separate from the RqstXML class because of the name collisions
 * between request and response class names in the XSD's.
 * 
 * @author lewit019
 */
...
<<<<<<< HEAD
  ...
  public static String getXML(DTITransactionTO responseIn) throws JAXBException {
    ...
    // Payload Header
    PayloadHeaderTO payloadHeaderTO = responseIn.getResponse().getPayloadHeader();
    PayloadHeader payloadHeader = PayloadHeaderXML.getJaxb(payloadHeaderTO, dtiErrorTO);
    ...
    if ((dtiErrorTO != null) && (dtiErrorTO.getErrorScope() == DTIErrorCode.ErrorScope.PAYLOAD)) {
      ...
      if ((dtiErrorTO != null) && (dtiErrorTO.getErrorScope() == DTIErrorCode.ErrorScope.TICKET)
          && (responseIn.getResponse().getCommandBody() == null)) {
        dtiErrorTO.setErrorScope(DTIErrorCode.ErrorScope.COMMAND);
      }

      cmd = new Command();
      CommandHeaderTO commandHeaderTO = responseIn.getResponse().getCommandHeader();
      CommandHeader commandHeader = CommandHeaderXML.getJaxb(commandHeaderTO, dtiErrorTO);
    ...
    // body. This is excepted for the price mismatch warning.
    if ((dtiErrorTO != null) && (dtiErrorTO.getErrorScope() != DTIErrorCode.ErrorScope.TICKET)) {

      if ((dtiErrorTO.getErrorCode().toString().compareTo(DTIErrorCode.PRICE_MISMATCH_WARNING.getErrorCode()) == 0)
          && (responseIn.getResponse().getCommandBody() != null)) {
   ...
   *           should any parsing errors occur.
   */
  private static void setCommandBodyXML(DTITransactionTO responseIn, Command cmd, DTIErrorTO errorTO)
      throws JAXBException {
    ...
    case QUERYTICKET:
      QueryTicketResponseTO queryRespTO = (QueryTicketResponseTO) responseIn.getResponse()
          .getCommandBody();
    ...
    case RESERVATION:
      ReservationResponseTO resRespTO = (ReservationResponseTO) responseIn.getResponse()
          .getCommandBody();
      ReservationResponse resResp = ReservationXML.getJaxb(resRespTO, errorTO);
      cmd.setReservationResponse(resResp);
      break;

    case UPGRADEALPHA:
      UpgradeAlphaResponseTO uaRespTO = (UpgradeAlphaResponseTO) responseIn.getResponse()
          .getCommandBody();
      UpgradeAlphaResponse uaResp = UpgradeAlphaXML.getJaxb(uaRespTO, errorTO);
      cmd.setUpgradeAlphaResponse(uaResp);
      break;

    case VOIDTICKET:
      VoidTicketResponseTO vtRespTO = (VoidTicketResponseTO) responseIn.getResponse()
          .getCommandBody();
      VoidTicketResponse vtResp = VoidTicketXML.getJaxb(vtRespTO, errorTO);
      cmd.setVoidTicketResponse(vtResp);
      break;

    case CREATETICKET:
      CreateTicketResponseTO ctRespTO = (CreateTicketResponseTO) responseIn.getResponse()
          .getCommandBody();
      CreateTicketResponse ctResp = CreateTicketXML.getJaxb(ctRespTO, errorTO);
      cmd.setCreateTicketResponse(ctResp);
      break;

    case UPDATETICKET:
      UpdateTicketResponseTO uTktRespTO = (UpdateTicketResponseTO) responseIn.getResponse()
          .getCommandBody();
      UpdateTicketResponse uTktResp = UpdateTicketXML.getJaxb(uTktRespTO, errorTO);
      cmd.setUpdateTicketResponse(uTktResp);
      break;
      
    case UPGRADEENTITLEMENT: // 2.10
    	UpgradeEntitlementResponseTO uEntRespTO = (UpgradeEntitlementResponseTO) responseIn.getResponse()
    		.getCommandBody();
    	UpgradeEntitlementResponse uEntResp = UpgradeEntitlementXML.getJaxb(uEntRespTO, errorTO);
    	cmd.setUpgradeEntitlementResponse(uEntResp);
    	break;
    	
    case QUERYRESERVATION:  
      QueryReservationResponseTO qResRespTO = (QueryReservationResponseTO) responseIn.getResponse()
          .getCommandBody();
      QueryReservationResponse qResResp = QueryReservationXML.getJaxb(qResRespTO, errorTO);
      cmd.setQueryReservationResponse(qResResp);
      break;
      
    case RENEWENTITLEMENT: // As of 2.16.1, JTL
      RenewEntitlementResponseTO qRenewRespTO = (RenewEntitlementResponseTO) responseIn.getResponse()
          .getCommandBody();
      RenewEntitlementResponse qRenewResp = RenewEntitlementXML.getJaxb(qRenewRespTO, errorTO);
   ...
   *           should any parsing errors occur.
   */
  private static void setEmptyCommandBodyXML(DTITransactionTO responseIn, Command cmd) throws JAXBException {
    TransactionType responseType = responseIn.getTransactionType();
    switch (responseType) {
