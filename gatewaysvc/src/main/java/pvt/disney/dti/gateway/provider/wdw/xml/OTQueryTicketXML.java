 ...
 * and back again for the Omni Ticket Query Ticket Command. Please note that all
 * of its methods are either package or private level access.
 * 
 * @author lewit019
 */
...
<<<<<<< HEAD
  ...
  private static EventLogger logger = EventLogger.getLogger(OTQueryTicketXML.class);

  /**
   * Adds the transaction body element of a query ticket to the XML of the
   * command stanza.
   * 
   * @param otQryTktTO
   ...
   */
  static void addTxnBodyElement(OTQueryTicketTO otQryTktTO, Element commandStanza) {
    ...
    // SiteNumber
    qryTktStanza.addElement("SiteNumber").addText(otQryTktTO.getSiteNumber().toString());
    ...
    for /* each */(OTTicketInfoTO otTicketInfo : /* in */tktList) {
      ...
      // Item
      ticketInfoStanza.addElement("Item").addText(otTicketInfo.getItem().toString());

      // TicketSearchMode
      Element tktSrchModeStanza = ticketInfoStanza.addElement("TicketSearchMode");
      ...
      case TDSSN:
        ...
        tdssnElement.addElement("Site").setText(otTicket.getTdssnSite());
        String station = otTicket.getTdssnStation();
        if (station.length() > 2) {
          String prefix = station.substring(0, 3);
          if (prefix.compareTo("CAS") == 0)
            tdssnElement.addElement("Station").setText(otTicket.getTdssnStation());
          else
            tdssnElement.addElement("Station").setText(CASPREFIX + otTicket.getTdssnStation());
        } else
          tdssnElement.addElement("Station").setText(CASPREFIX + otTicket.getTdssnStation());
      ...
      case EXTERNALTICKETCODE:
        tktSrchModeStanza.addElement("ExternalTicketCode")
            .addText(otTicket.getExternalTicketCode());
        break;
      }
  ...
  /**
   * Gets the transfer object for query ticket based on the parsed XML passed
   * in.
   * 
   * @param queryTktNode
  ...
  /**
   * Sets the ticket info list transfer objects based upon the parsed XML that's
   * passed in.
   * 
  ...
  private static void setOTTicketInfoTOList(List<Node> ticketInfoNodeList,
    ...
    for /* each */(Node aNode : /* in */ticketList) {
      ...
      if (itemNode == null)
        throw new DTIException(OTQueryTicketXML.class, DTIErrorCode.TP_INTERFACE_FAILURE,
      ...
      if (seasonPassInfoNode != null) {
        ...
        Node demoDataNode = seasonPassInfoNode.selectSingleNode("DemographicData");
        if (demoDataNode != null) {
          ...
          for (/*each*/Node aFieldNode: /* in */ aFieldList ) {
            ...
            String fieldIdString = aFieldNode.selectSingleNode("FieldId").getText();
            String fieldValueString = aFieldNode.selectSingleNode("FieldValue").getText();
            ...
            try {
              ...
              logger.sendEvent("Invalid FieldId sent by Iago.  Check standard err logs.", 
                  EventType.WARN, new OTQueryTicketXML());
              nfe.printStackTrace();
            }

            OTFieldTO newOTField = new OTFieldTO(fieldId,fieldValueString);
      ...
      Node denyMultiEntitlementFunctionsNode = aNode.selectSingleNode("DenyMultiEntitlementFunctions");
      ...
      if (accountIdNode != null) {
    	  ...
    	  try {
    		  otTktInfoTO.setAccountId(new BigInteger(inText));
    	  } catch(NumberFormatException nfe) {
    		  logger.sendEvent("Invalid QueryTicket AccountId sent by Iago:"+inText+". Check standard err logs.", 
    				  EventType.WARN, new OTQueryTicketXML());
    		  nfe.printStackTrace();
    	  }
      }
