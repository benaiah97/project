...
<<<<<<< HEAD
  ...
  private final static String[] UA_TAGS = { "UpgradeTicketInfo", "TransactionDSSN",
      "TransactionCOD", "PaymentInfo" };
  ...
  static String transformRequest(DTITransactionTO dtiTxn) throws DTIException {
    ...
    // === Header Level ===
    OTHeaderTO hdr = WDWBusinessRules.transformOTHeader(dtiTxn, REQUEST_TYPE_UPGRADE,
        REQUEST_SUBTYPE_UPGRADETICKET);
    ...
    if (anAttributeTO == null) {
    	...
    	otUpgrdTkt.setSiteNumber(Integer.parseInt(anAttributeTO.getAttrValue()));
    }

    HashMap<String, DBProductTO> dtiProdMap = dtiTxn.getDbProdMap();

    // UpgradeTicketInfo
    ArrayList<OTUpgradeTicketInfoTO> otUpgrdTktList = otUpgrdTkt.getUpgradeTicketInfoList();
    ...
    if (otPaymentList.size() == 0) {
      throw new DTIException(WDWBusinessRules.class, DTIErrorCode.INVALID_PAYMENT_AMOUNT,
          dtiTxn.getEntityTO().getTsMac() + "/" + dtiTxn.getEntityTO().getTsLocation() +
          " did not provide any payments for upgrade alpha or did not have a valid " +
          "default paytype for upgrade alpha.");      
    }

    // SellerId
    EntityTO entTO = dtiTxn.getEntityTO();
    if (entTO.getDefSalesRepId() != 0)
      otUpgrdTkt.setSellerId(BigInteger.valueOf(entTO.getDefSalesRepId()));
    ...
    // IATA
    if (entTO.getDefIata() != null)
      otUpgrdTkt.setIATA(entTO.getDefIata());
    ...
    otUpgrdTkt.setTransactionNote(dtiRequest.getPayloadHeader().getPayloadID());

    // InTransaction Attribute
    ArrayList<OTInTransactionAttributeTO> inTxnAttrList = otUpgrdTkt.getInTxnAttrList();
  ...
  /**
   * Transforms a upgrade ticket response string from the WDW provider and
   * updates the DTITransactionTO object with the response information.
   * NOTE:  As of 2.12, validates that the number of tickets coming back
   * in match what was sent.
   * 
   * @param dtiTxn
  ...
  static void transformResponseBody(DTITransactionTO dtiTxn, OTCommandTO otCmdTO,
    ...
    if ((inCount != outCount) && (outCount > 0)) {
    	throw new DTIException(IagoClient.class, DTIErrorCode.TP_INTERFACE_FAILURE,
  				"Number of tickets returned  (" + outCount + ") don't match number sent (" + inCount + ")." );	
    }
    
    // Ticket
    ArrayList<OTUpgradeTicketInfoTO> otUpgrdTktInfoList = otUpgrdTktResp.getUpgradeTicketInfoList();
    ...
    if (otUpgrdTktInfoList.size() > 0) {
      ...
      for /* each */(OTUpgradeTicketInfoTO otUpgrdTktInfo : /* in */otUpgrdTktInfoList) {
        ...
        dtiTkt.setDssn(otTkt.getTdssnDate(), otTkt.getTdssnSite(), otTkt.getTdssnStation(), otTkt
            .getTdssnTicketId());
        ...
        OTTransactionDSSNTO otTxnDSSN = otUpgrdTktResp.getTransactionDSSN();
    ...
    for /* each */(DBProductTO dbProd : /* in */dbProdList) {
      ...
      // ProdReceiptMsg
      String prodReceiptMsg = ProductKey.getProductReceipt(dbProd.getPdtCode());
   ...
   * RULE: Are ticket shells provided on order active? RULE: Are ticket shells
   * provided on order associated to the right product? <BR>
  ...
  public static void applyWDWUpgradeAlphaRules(DTITransactionTO dtiTxn) throws DTIException {
    ...
    UpgradeAlphaRequestTO reqTO = (UpgradeAlphaRequestTO) dtiTxn.getRequest().getCommandBody();
    ...
    HashSet<Integer> orderShellSet = WDWBusinessRules.validateTicketOrderShells(aTktListTO);
    if (orderShellSet.size() > 0) {
      ArrayList<Integer> activeShells = ProductKey.getActiveShells(orderShellSet);
      WDWBusinessRules.validateTicketShellActive(orderShellSet, activeShells);
