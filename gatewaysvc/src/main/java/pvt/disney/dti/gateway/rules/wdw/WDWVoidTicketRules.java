...
<<<<<<< HEAD
  ...
  private final static String[] VOID_TAGS = { "TicketInfoVoid", "TransactionDSSN",
  ...
  static String transformRequest(DTITransactionTO dtiTxn) throws DTIException {
    ...
    // === Command Level ===
    OTCommandTO atsCommand = new OTCommandTO(OTCommandTO.OTTransactionType.VOIDTICKET);
    ...
    // === Header Level ===
    OTHeaderTO hdr = WDWBusinessRules.transformOTHeader(dtiTxn, REQUEST_TYPE_VOID,
        REQUEST_SUBTYPE_VOIDTICKET);
    ...
    if (anAttributeTO == null) {
    	...
    	otVoidTkt.setSiteNumber(Integer.parseInt(anAttributeTO.getAttrValue()));
    }
    ...
    otVoidTkt.setTransactionNote(dtiTxn.getRequest().getPayloadHeader().getPayloadID());
    ...
    /*
     * Note, while code was in the old gateway to populate
     * VoidTicket.AssociationInfo.AssociationId and MemberId, it was being done
     * from environment variables that, on first examination, where populated by
     * a product SQL query. Upon detailed examination of the code, no product
     * query associated with void ticket could be located, so it is presumed
     * that although the code was present in the old gateway, it was never
     * executed. Hence, it has been removed here. JTL
     */

    // InTransactionAttributes
    ArrayList<OTInTransactionAttributeTO> inTxnAttrList = otVoidTkt.getInTxnAttrList();
  ...
  /**
   * Transforms a void ticket response string from the WDW provider and updates
  ...
  static void transformResponseBody(DTITransactionTO dtiTxn, OTCommandTO otCmdTO,
    ...
    if ((otVoidTktResp.getVTicketList() != null) && (otVoidTktResp.getVTicketList().size() > 0)) {
    ...
    if (otVoidTktResp.getTransactionDSSN() != null) {
      ...
      dtiTktTxn.setDssn(otTxnDSSN.getDate(), otTxnDSSN.getSite(), otTxnDSSN.getStation(), otTxnDSSN
  ...
  public static void applyWDWVoidTicketRules(DTITransactionTO dtiTxn) throws DTIException {
    ...
    VoidTicketRequestTO reqTO = (VoidTicketRequestTO) dtiTxn.getRequest().getCommandBody();
