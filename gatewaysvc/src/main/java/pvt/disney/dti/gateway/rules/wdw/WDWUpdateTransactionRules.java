...
<<<<<<< HEAD
  ...
  static String transformRequest(DTITransactionTO dtiTxn) throws DTIException {
    ...
    // === Command Level ===
    OTCommandTO atsCommand = new OTCommandTO(OTCommandTO.OTTransactionType.UPDATETRANSACTION);
    ...
    // === Header Level ===
    OTHeaderTO hdr = WDWBusinessRules.transformOTHeader(dtiTxn, REQUEST_TYPE_UPDATE,
        REQUEST_SUBTYPE_UPDATE_TXN);
    ...
    if (anAttributeTO == null) {
    	...
    	otUpdtTxn.setSiteNumber(Integer.parseInt(anAttributeTO.getAttrValue()));
    }
    ...
    if (dtiTktTxn.getTxnIdType() == TicketTransactionTO.TransactionIDType.TRANSDSSN) {
      ...
      otTxnDSSN.setTransactionId(Integer.decode(dtiTktTxn.getDssnNumber()));
    ...
    otUpdtTxn.setTransactionNote(dtiRequest.getPayloadHeader().getPayloadID());
    ...
    ArrayList<OTInTransactionAttributeTO> inTxnAttrList = otUpdtTxn.getInTxnAttrList();
   ...
   * case, there are no fields which need to be transformed.
   * 
   * @param dtiTxn
