...
<<<<<<< HEAD
   ...
   * their correct DLRTransactionNamed class. This method is primarily for
  ...
  /**
   * This method changes from the DTITransaction to the DLR provider's request
   * string format.
   * 
   * @param dtiTxn
   ...
   */
  public static String changeToDLRProviderFormat(DTITransactionTO dtiTxn)
      throws DTIException {
  ...
  /**
   * Parses the xmlResponse string from the DLR provider into the
  ...
  public static DTITransactionTO changeDLRProviderFormatToDti(
    ...
    case UPGRADEALPHA:
      dtiTxn = DLRUpgradeAlphaRules.transformResponse(dtiTxn, xmlResponse);
      break;
    ...
    case QUERYRESERVATION:
      dtiTxn = DLRQueryReservationRules.transformResponse(dtiTxn, xmlResponse);
      break;
   ...
   * @throws DTIException
   */
  static void validateInBoundDLRTickets(ArrayList<TicketTO> aTktList)
      throws DTIException {
    ...
    for (TicketTO aTicketTO : aTktList) {
      ...
      if (aTicketTO.getTicketTypes().size() != 1)
        throw new DTIException(DLRBusinessRules.class,
            DTIErrorCode.INVALID_TICKET_ID,
            "In-bound dLR txn with <> 1 TktId: "
      ...
      if (aTicketTO.getTicketTypes().get(0) != TicketTO.TicketIdType.EXTERNAL_ID) {
        throw new DTIException(DLRBusinessRules.class,
            DTIErrorCode.INVALID_TICKET_ID,
            "In-bound dLR txn with an invalid TicketIdType: "
                + aTicketTO.getTicketTypes().get(0));
      } else {
        if (aTicketTO.getExternal().length() > MAX_VISUALID_LENGTH)
          throw new DTIException(DLRBusinessRules.class,
              DTIErrorCode.INVALID_TICKET_ID,
              "In-bound DLR txn with invalid External (VisualID) length: "
                  + aTicketTO.getExternal().length());
      }
