 ...
 * 
 */
public class GWTicketActivationVoidXMLTestCase {
	...
	/**
	 * Test getting xml from an sample egalaxy xml file.
	 */
	@Test
	public final void testGetXML() {
		...
		String fileName = new String(
				"./src/test/resources/xml/egalaxy/VoidTicketRequest.xml");
		...
		GWEnvelopeTO dtiTxn = createCancelTicketActivationReqTO();
		String newXML = null;
		...
		System.out.println(DLRTestUtil2.prettyPrint(taRespNew));
	}

	/**
	 * Test get transfer object for a successful TicketActivation.Command =
	 * Cancel, aka a void ticket.
	 */
	@Test
	public final void testGetSucessTO() {
		...
		String fileName = new String(
				"./src/test/resources/xml/egalaxy/VoidTicketResponse.xml");
		...
		try {
			...
			validateVoidTicketRespTO(parsedEnvTO);
	...
	/**
	 * Test get transfer object for an error reponse, ticket not voidable.
	 */
	@Test
	public final void testGetTOAlreadyVoidedError() {

		// ***** Read the test file. *****
		String inputXML = null;
		String fileName = new String(
				"./src/test/resources/xml/egalaxy/VoidTicketResponse-Error1420-AlreadyVoided.xml");
		...
		try {
			validateVoidTicketRespTOError(parsedEnvTO,
					DLRTestUtil.TICKET_ACTIVATION_ALREADY_VOIDED_CODE);
		} catch (Exception e) {
			fail("Exception parsing the gateway response XML: " + e.toString());
		}
	 ...
	 */
	@Test
	public final void testGetTOInvalidTicketError() {

		// ***** Read the test file. *****
		String inputXML = null;
		String fileName = new String(
				"./src/test/resources/xml/egalaxy/VoidTicketResponse-Error1404-TcktDoesNotExist.xml");
		...
		try {
			validateVoidTicketRespTOError(parsedEnvTO,
					DLRTestUtil.TICKET_ACTIVATION_DOES_NOT_EXIST_CODE);
		} catch (Exception e) {
			fail("Exception parsing the gateway response XML: " + e.toString());
		}
	 ...
	 */
	@Test
	public final void testGetTONotPermittedError() {

		// ***** Read the test file. *****
		String inputXML = null;
		String fileName = new String(
				"./src/test/resources/xml/egalaxy/VoidTicketResponse-Error1403-MessageNotPermitted.xml");
		...
		try {
			validateVoidTicketRespTOError(parsedEnvTO,
					DLRTestUtil.TICKET_ACTIVATION_NOT_PERMITTED_CODE);
		} catch (Exception e) {
			fail("Exception parsing the gateway response XML: " + e.toString());
		}
	...
	/**
	 * Helper method
	 * Creates a ticket activation cancel (void ticket) request transfer object.
	 * 
	 * @return the gW envelope to
	 */
	static GWEnvelopeTO createCancelTicketActivationReqTO() {
		...
		gwTktActTO.addTicket(DLRTestUtil.VISUALID);
		gwTktActTO.setCustomerID(DLRTestUtil.CUSTOMERID);
		gwTktActTO.setCommand(TransformConstants.GW_COMMAND_CANCEL);
	...
	/**
	 * Validates a gateway galaxy TicketActivation.Command = Cancel (void
	 * ticket) response transfer object.
	 * 
	 * @param envTo
	 ...
	 *             the exception
	 */
	static void validateVoidTicketRespTO(GWEnvelopeTO envTo) throws Exception {
	...
	/**
	 * Validates a gateway galaxy ticket activation (void ticket) response
	 ...
	 *             the exception
	 */
	private static void validateVoidTicketRespTOError(GWEnvelopeTO envTo,
			String ticketErrorCode) throws Exception {
		GWHeaderTO gwHdrTO = envTo.getHeaderTO();
		validateRespHeaderTO(gwHdrTO);
		// Body
		GWBodyTO gwBodyTO = envTo.getBodyTO();
		...
		//  visualID should be added to errors TO
	...
	private static void validateRespHeaderTO(GWHeaderTO hdrTo) throws Exception {
		...
		if (messageId.compareTo(DLRTestUtil.MESSAGEID) != 0) {
			...
			throw new Exception("Unexpected value of " + messageId.toString()
					+ " in Header/MessageID");
		}
		...
		if (msgType.compareTo(DLRTestUtil.MESSAGETYPE_TICKET_ACTIVATION_RSP) != 0)
			throw new Exception("Unexpected value of " + msgType
		...
		if (sourceId.compareTo(DLRTestUtil.SOURCEID) != 0) {
			throw new Exception("Unexpected value of " + sourceId
					+ " in Header/SourceID.");
		}
		...
		GregorianCalendar gCalBase = UtilityXML
				.getGCalFromEGalaxyDate(DLRTestUtil.TIMESTAMPRSP);
		if (gCalTO.compareTo(gCalBase) != 0) {
			throw new Exception("Unexpected value of " + gCalTO.toString()
					+ " in Header/TimeStamp.");
		}
	}
}
